...
from lahm.agents.utils.agent_states import (
    AgentState,
    InvestDebateState,
    RiskDebateState,
)
from lahm.dataflows.interface import set_config
...
class LahmGraph:
    ...
    def propagate(self, company_name, trade_date, progress_callback=None, task_id=None):
        ...
        # 添加详细的接收日志
        logger.debug(f"🔍 [GRAPH DEBUG] ===== LahmGraph.propagate 接收参数 =====")
    ...
    def _log_state(self, trade_date, final_state):
        ...
        # Save to file
        directory = Path(f"eval_results/{self.ticker}/LahmStrategy_logs/")
        directory.mkdir(parents=True, exist_ok=True)

        with open(
            f"eval_results/{self.ticker}/LahmStrategy_logs/full_states_log.json",
            "w",
        ) as f:
