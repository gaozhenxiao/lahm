...
# 导入统一新闻工具
from lahm.tools.unified_news_tool import create_unified_news_tool
# 导入股票工具类
from lahm.utils.stock_utils import StockUtils
# 导入Google工具调用处理器
from lahm.agents.utils.google_tool_handler import GoogleToolCallHandler
from lahm.agents.utils.instrument_utils import build_instrument_context
from lahm.agents.utils.core_analysis_framework import NEWS_ANALYSIS_FOCUS
...
def create_news_analyst(llm, toolkit):
   ...
   # 创建统一新闻工具
        ...
        system_message = (
            "您是一位财经新闻分析师。任务不是煽情解读股价涨跌，而是判断新闻是否改变公司经营实质。\n\n"
            "主要职责：\n"
            "1. 获取最新相关新闻（优先近时）\n"
            "2. 筛掉噪音，只保留可能改变经营预期的信息\n"
            "3. 把每条重要新闻映射到经营五维\n\n"
            f"{NEWS_ANALYSIS_FOCUS}\n\n"
            "重点新闻类型：需求/订单、竞争与份额、产能与供应链、价格与出货、利润率、"
            "股东增减持/质押/回购、重大政策与并购。\n\n"
            "报告结构：\n"
            "## 重要新闻摘要（按时间）\n"
            "## 对经营五维的影响（需求/竞争/产供销/量价利/股东）\n"
            "## 综合判断（短）\n\n"
            "要求：中文；写清证据与变化；信息不足写「暂无可靠证据」；不写技术指标；"
            "末尾附 Markdown 表格总结。"
        )
            ...
            try:
                ...
                if pre_fetched_news and len(pre_fetched_news.strip()) > 100:
                    ...
                    analysis_system_prompt = (
                        "您是一位财经新闻分析师。基于已提供的新闻，判断其是否改变经营实质。\n"
                        f"{NEWS_ANALYSIS_FOCUS}\n"
                        "新闻数据已提供，无需调用工具。用中文按五维写清变化与证据。"
                    )

                    enhanced_prompt = f"""请基于以下新闻，对 {company_name}（{ticker}）做经营向解读：
