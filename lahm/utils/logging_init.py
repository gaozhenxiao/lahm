...
from lahm.utils.logging_manager import setup_logging, get_logger
...
def init_logging(config_override: Optional[dict] = None) -> None:
    ...
    # 获取初始化日志器
    logger = get_logger('lahm.init')
    
    # 记录初始化信息
    logger.info("🚀 柳暗花明 日志系统初始化完成")
...
def log_startup_info():
    """记录应用启动信息"""
    logger = get_logger('lahm.startup')
    ...
    env_info = {
        ...
        'LAHM_LOG_LEVEL': os.getenv('LAHM_LOG_LEVEL', 'INFO'),
        'LAHM_LOG_DIR': os.getenv('LAHM_LOG_DIR', './logs'),
    }
...
def log_shutdown_info():
    """记录应用关闭信息"""
    logger = get_logger('lahm.shutdown')
