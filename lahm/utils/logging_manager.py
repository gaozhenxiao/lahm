...
_bootstrap_logger = logging.getLogger("lahm.logging_manager")
...
class LahmLogger:
    ...
    def _load_default_config(self) -> Dict[str, Any]:
        ...
        # 从环境变量获取配置
        log_level = os.getenv('LAHM_LOG_LEVEL', 'INFO').upper()
        log_dir = os.getenv('LAHM_LOG_DIR', './logs')
    ...
    def _add_structured_handler(self, logger: logging.Logger):
        ...
        log_file = log_dir / 'lahm_structured.log'
...
# 全局日志管理器实例
_logger_manager: Optional[LahmLogger] = None


def get_logger_manager() -> LahmLogger:
    """获取全局日志管理器实例"""
    global _logger_manager
    if _logger_manager is None:
        _logger_manager = LahmLogger()
...
def setup_logging(config: Optional[Dict[str, Any]] = None):
    """设置项目日志系统（便捷函数）"""
    global _logger_manager
    _logger_manager = LahmLogger(config)
    return _logger_manager
