...
# 运行时设置：读取系统时区
from lahm.config.runtime_settings import get_timezone_name
...
class ConfigManager:
    ...
    def _init_mongodb_storage(self):
        ...
        try:
            ...
            # 自动构建：当 MONGODB_CONNECTION_STRING 未设置时，从 MONGODB_HOST/PORT/USERNAME/PASSWORD 构建
            if not connection_string:
                mongo_host = os.getenv("MONGODB_HOST", "127.0.0.1")
                mongo_port = os.getenv("MONGODB_PORT", "27017")
                mongo_user = os.getenv("MONGODB_USERNAME", "admin")
                mongo_password = os.getenv("MONGODB_PASSWORD", "lahm123")
                mongo_auth = os.getenv("MONGODB_AUTH_SOURCE", "admin")
                if mongo_user and mongo_password:
                    connection_string = f"mongodb://{mongo_user}:{mongo_password}@{mongo_host}:{mongo_port}/?authSource={mongo_auth}"
                else:
                    connection_string = f"mongodb://{mongo_host}:{mongo_port}"
                os.environ["MONGODB_CONNECTION_STRING"] = connection_string
                logger.info(f"🔍 [ConfigManager] 自动构建 MONGODB_CONNECTION_STRING: {mongo_host}:{mongo_port}")
            database_name = os.getenv("MONGODB_DATABASE_NAME", "lahm")
    ...
    def _init_default_configs(self):
        ...
        if not self.settings_file.exists():
            ...
            default_settings = {
                ...
                "results_dir": os.path.join(os.path.expanduser("~"), "Documents", "柳暗花明", "results"),  # 结果目录
    ...
    def load_settings(self) -> Dict[str, Any]:
        ...
        try:
            if self.settings_file.exists():
                ...
                settings = {
                    ...
                    "cache_dir": os.path.join(os.path.expanduser("~"), "Documents", "柳暗花明", "data", "cache"),
                    "results_dir": os.path.join(os.path.expanduser("~"), "Documents", "柳暗花明", "results"),
        ...
        env_settings = {
            ...
            "results_dir": os.getenv("LAHM_RESULTS_DIR", ""),
            "log_level": os.getenv("LAHM_LOG_LEVEL", "INFO"),
            "data_dir": os.getenv("LAHM_DATA_DIR", ""),  # 数据目录环境变量
            "cache_dir": os.getenv("LAHM_CACHE_DIR", ""),  # 缓存目录环境变量
        }
...
def _get_project_config_dir():
    """获取项目根目录的配置目录"""
    # 从当前文件位置推断项目根目录
    current_file = Path(__file__)  # lahm/config/config_manager.py
