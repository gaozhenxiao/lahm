#!/usr/bin/env python3
"""
App 缓存读取适配器（柳暗花明 -> app MongoDB 集合）
...
def get_basics_from_cache(stock_code: Optional[str] = None) -> Optional[Dict[str, Any] | List[Dict[str, Any]]]:
    ...
    try:
        ...
        try:
            ...
            from lahm.config.database_manager import get_database_manager  # type: ignore
            db_name = get_database_manager().mongodb_config.get("database", "lahm")
        except Exception:
            db_name = "lahm"
        db = client[db_name]
