...
def write_md(rows, path: Path, title: str) -> None:
    md = [
        title,
        "",
        "成本：佣金万一 + 印花税千一。思路偏另类（NR7/日历/波动扩张/缩量推力/缺口回补等）。",
        "",
...
def main() -> None:
    ...
    run(
        [
            ...
            only,
            "--out",
            "data/factors/wave5_smoke_summary.json",
        ]
    )
    smoke = rank_rows(ROOT / "data/factors/wave5_smoke_summary.json")
    ...
    run(
        [
            ...
            ",".join(keep),
            "--out",
            "data/factors/wave5_full_summary.json",
        ]
    )
    try:
        import matplotlib
    ...
    rows = rank_rows(ROOT / "data/factors/wave5_full_summary.json")
    write_md(rows, ROOT / "data/factors/wave5_rank.md", "# 第五波另类因子全量回测（沪深300，2018–2026）")
    ...
    smoke_data = json.loads((ROOT / "data/factors/wave5_smoke_summary.json").read_text(encoding="utf-8"))
    ...
    (ROOT / "data/factors/wave5_keep.json").write_text(
