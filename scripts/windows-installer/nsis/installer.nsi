...
!endif
!ifndef PACKAGE_ZIP
  !define PACKAGE_ZIP "C:\\Lahm\\release\\packages\\Lahm-Portable-latest.zip"
!endif
!ifndef OUTPUT_DIR
  !define OUTPUT_DIR "C:\\Lahm\\release\\packages"
!endif

Name "${PRODUCT_NAME}"
OutFile "${OUTPUT_DIR}\LahmSetup-${PRODUCT_VERSION}.exe"
InstallDir "C:\Lahm"
...
!define MUI_FINISHPAGE_RUN_TEXT "Launch Lahm"
...
nsExec::ExecToLog 'powershell -ExecutionPolicy Bypass -Command "Expand-Archive -Path \"$INSTDIR\Lahm-Portable-latest.zip\" -DestinationPath \"$INSTDIR\" -Force"'
Pop $0
...
Delete "$INSTDIR\Lahm-Portable-latest.zip"
...
CreateDirectory "$SMPROGRAMS\Lahm"
...
nsExec::ExecToLog 'powershell -ExecutionPolicy Bypass -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut(\"$SMPROGRAMS\Lahm\Start Lahm.lnk\"); $s.TargetPath = \"powershell.exe\"; $s.Arguments = \"-ExecutionPolicy Bypass -NoExit -Command `\"Set-Location -Path ''$INSTDIR''; & ''$INSTDIR\start_all.ps1''`\"\"; $s.WorkingDirectory = \"$INSTDIR\"; $s.Save(); $bytes = [System.IO.File]::ReadAllBytes($s.FullName); $bytes[0x15] = $bytes[0x15] -bor 0x20; [System.IO.File]::WriteAllBytes($s.FullName, $bytes)"'

nsExec::ExecToLog 'powershell -ExecutionPolicy Bypass -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut(\"$SMPROGRAMS\Lahm\Stop Lahm.lnk\"); $s.TargetPath = \"powershell.exe\"; $s.Arguments = \"-ExecutionPolicy Bypass -NoExit -Command `\"Set-Location -Path ''$INSTDIR''; & ''$INSTDIR\stop_all.ps1''`\"\"; $s.WorkingDirectory = \"$INSTDIR\"; $s.Save(); $bytes = [System.IO.File]::ReadAllBytes($s.FullName); $bytes[0x15] = $bytes[0x15] -bor 0x20; [System.IO.File]::WriteAllBytes($s.FullName, $bytes)"'

nsExec::ExecToLog 'powershell -ExecutionPolicy Bypass -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut(\"$DESKTOP\Lahm.lnk\"); $s.TargetPath = \"powershell.exe\"; $s.Arguments = \"-ExecutionPolicy Bypass -NoExit -Command `\"Set-Location -Path ''$INSTDIR''; & ''$INSTDIR\start_all.ps1''`\"\"; $s.WorkingDirectory = \"$INSTDIR\"; $s.Save(); $bytes = [System.IO.File]::ReadAllBytes($s.FullName); $bytes[0x15] = $bytes[0x15] -bor 0x20; [System.IO.File]::WriteAllBytes($s.FullName, $bytes)"'

; Write uninstaller
WriteUninstaller "$INSTDIR\Uninstall.exe"

; Registry entries for Control Panel uninstall
WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Lahm" "DisplayName" "Lahm"
WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Lahm" "UninstallString" "$INSTDIR\Uninstall.exe"
WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Lahm" "DisplayVersion" "${PRODUCT_VERSION}"
WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Lahm" "InstallLocation" "$INSTDIR"

DetailPrint "Installation completed!"
SectionEnd

Section "Uninstall"
 Delete "$SMPROGRAMS\Lahm\Start Lahm.lnk"
 Delete "$SMPROGRAMS\Lahm\Stop Lahm.lnk"
 RMDir  "$SMPROGRAMS\Lahm"
 Delete "$DESKTOP\Lahm.lnk"
 Delete "$INSTDIR\Uninstall.exe"
 DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Lahm"
 RMDir /r "$INSTDIR"
SectionEnd