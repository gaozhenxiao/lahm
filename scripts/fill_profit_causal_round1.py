"""把利润因果链 Round1 Top 候选写入 lahm（仅 INSERT）。

来源：data/factors/mine_profit_causal_round1/{univ}/results.json
行情：腾讯 qfq（_shared/daily）；BaoStock 禁用。
硬性：不 update 已有 factor_id；不填空洞；撞号 abort。
MIN_UI=201（主库已至 #200；不覆盖 #188/#189）。
宁少勿滥：3 条正交因果族（应收 / 合同负债二阶 / 费用率）。
"""
...
MINE_ROOT = ROOT / "data" / "factors" / "mine_profit_causal_round1"
MIN_UI = 201
...
CANDIDATES: List[Dict[str, Any]] = [
    {
        "factor_id": "ar_tighten_ar015_hs300_causal",
        "cfg_id": "ar_tighten__ar015_ry05_brk60_h40",
        "univ": "hs300",
        "signal": sig.signal_ar_tighten_rev_break,
        "signal_name": "signal_ar_tighten_rev_break",
        "need_profit": True,
        "need_growth": False,
        "need_balance": False,
        "need_fin_db": True,
    ...
    {
        "factor_id": "cl_yoy_acc_clacc12_csi500_causal",
        "cfg_id": "cl_yoy_acc__clacc12_y15_brk70_h45",
        "univ": "csi500",
        "signal": sig.signal_cl_yoy_accel_break,
        "signal_name": "signal_cl_yoy_accel_break",
        "need_profit": True,
        "need_growth": False,
        "need_balance": False,
        "need_fin_db": True,
    ...
    {
        "factor_id": "opex_rev_ox05_hs300_causal",
        "cfg_id": "opex_rev__ox05_racc05_g06_brk60_h40",
        "univ": "hs300",
        "signal": sig.signal_opex_down_rev_accel_break,
        "signal_name": "signal_opex_down_rev_accel_break",
        "need_profit": True,
        "need_growth": False,
        "need_balance": False,
        "need_fin_db": True,
...
def _desc(c: dict, params: dict, mine: dict) -> str:
    return (
        f"mine_profit_causal_round1 / {c['univ']} `{c['cfg_id']}`"
        f"（时间加权 tw≈{mine.get('tw_score'):.3f}；利润因果 L2 R1）。"
    ...
    for dbn, planned in plans.items():
        ...
        for fid, created_at, ui in planned:
            ...
            payload = {
                ...
                "mine_meta": {
                    "source": f"mine_profit_causal_round1/{c['univ']}",
                    "cfg_id": c["cfg_id"],
...
def main() -> None:
    ...
    out = ROOT / "data" / "factors" / "fill_profit_causal_round1.json"
    payload = {
    ...
    if summary_md.exists():
        extra = [
            "",
            "## 本轮已入库（#201–#203；不覆盖 #188/#189；#198–#200 YoY 保留）",
            "",
        ...
        extra.extend(
            [
                "",
                "未挂：第二档 ar_tighten（与 #201 同族）；gp_rev_dual（tw 尚可但优先三族正交）；"
                "CSI1000 全样本 Sharpe 偏弱。",
                "",
            ]
        )
