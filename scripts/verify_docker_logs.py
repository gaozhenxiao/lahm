...
def check_container_status():
    ...
    if success:
        ...
        # 检查web容器是否运行
        if "柳暗花明-web" in output and "Up" in output:
            return True
        else:
            print("❌ 柳暗花明-web容器未正常运行")
            return False
    else:
...
os.environ['LAHM_LOG_DIR'] = '/app/logs'
...
"'''
    ...
    success, output, error = run_command(f"docker exec 柳暗花明-web {test_cmd}")
...
def check_container_logs():
    ...
    success, output, error = run_command("docker exec 柳暗花明-web ls -la /app/logs/")
    ...
    if success:
        ...
        # 检查具体的日志文件
        success2, output2, error2 = run_command("docker exec 柳暗花明-web find /app/logs -name '*.log*' -type f")
        if success2 and output2.strip():
            ...
            for log_file in output2.strip().split('\n'):
                if log_file.strip():
                    ...
                    # 获取文件大小
                    success3, output3, error3 = run_command(f"docker exec 柳暗花明-web wc -c {log_file}")
                    if success3:
...
def check_docker_stdout_logs():
    ...
    success, output, error = run_command("docker logs --tail 20 柳暗花明-web")
...
def main():
    ...
    if passed == len(results):
        print("\n🎉 所有检查都通过！日志功能正常")
        print("\n💡 现在可以:")
        print("   - 查看实时日志: tail -f logs/lahm.log")
