"""把结构大批量 Round2 Top 候选写入 lahm（仅 INSERT，≥#220）。

来源：data/factors/mine_struct_mass_round2/global_top.json + {univ}/results.json
自动按族正交选 12–20 条；近2年大亏剔除；宁可用略弱占位也不要只交三五个。
行情：腾讯 qfq；BaoStock 禁用。硬性：不 update；不填空洞；撞号 abort。

用法（挖掘完成后）:
  .venv\\Scripts\\python.exe scripts/fill_struct_mass_round2.py
  .venv\\Scripts\\python.exe scripts/fill_struct_mass_round2.py --dry-run
  .venv\\Scripts\\python.exe scripts/fill_struct_mass_round2.py --n 16
"""
...
MINE_ROOT = ROOT / "data" / "factors" / "mine_struct_mass_round2"
MIN_UI = 220
...
def _make_factor_id(univ: str, family: str, cfg_id: str) -> str:
    ...
    return f"{fam}_{short}_{univ}_sm2"


def _pick_candidates(n: int, min_tw: float) -> List[Dict[str, Any]]:
    top_path = MINE_ROOT / "global_top.json"
    if not top_path.exists():
        raise SystemExit(f"missing {top_path}; run mine_struct_mass_round2.py first")
        ...
        out.append(
            {
                ...
                "need_balance": nb,
                "need_fin_db": nf,
                "name": f"{tag_cn}({univ.upper()}·{_short_cfg(cfg)}·结构大批量R2)",
                "title": f"{fam} {univ} sm2",
                "tags": [
                    "结构",
                    tag_cn,
                    "突破",
...
def _py_lit(v: Any) -> str:
    ...
    if isinstance(v, dict):
        inner = ", ".join(f"{k}={_py_lit(val)}" for k, val in v.items())
        return f"dict({inner})" if v else "{}"
    if isinstance(v, (list, tuple)):
        return "[" + ", ".join(_py_lit(x) for x in v) + "]"
    if v is None:
        return "None"
    return repr(v)


def _signal_ref(fn: Any) -> str:
...
def _patch_factor_registry(candidates: List[dict], params_map: Dict[str, dict]) -> None:
    """把本轮因子永久写入 FACTOR_IMPL（ensure_builtins / 产物索引依赖）。"""
    reg = ROOT / "app" / "services" / "factors" / "factor_registry.py"
    text = reg.read_text(encoding="utf-8")
    marker = "\n\ndef compute_factor_signal("
    if marker not in text:
        raise SystemExit("ABORT: compute_factor_signal marker missing in factor_registry.py")
    blocks: List[str] = [
    ...
    for c in candidates:
        fid = c["factor_id"]
        if f'"{fid}":' in text:
            print(f"[registry] skip existing {fid}", flush=True)
            continue
        params = {k: v for k, v in params_map[fid].items() if not str(k).startswith("_")}
        # 去掉与 _COMMON 重复的默认项（保留关键覆盖）
        skip_defaults = {
            "universe",
            "price_start",
            "max_positions",
        ...
        # 实际仍写出全部关键参数，风格对齐 sm1
        kw = []
        for k, v in params.items():
            if k in ("position_logic",):
                continue
            kw.append(f"            {k}={_py_lit(v)},")
        tags = ", ".join(_py_lit(t) for t in c["tags"])
        block = (
            f'    "{fid}": {{\n'
            f'        "name": {_py_lit(c["name"])},\n'
            f'        "category": "fundamental",\n'
            f'        "description": (\n'
            f'            "mine_struct_mass_round2：{c["univ"]} `{c["cfg_id"]}` / {c["family"]}。"\n'
            f'            "结构信号+技术确认；腾讯 qfq；静态成分幸存者偏差。"\n'
            f"        ),\n"
            f'        "tags": [{tags}],\n'
            f'        "title": {_py_lit(c["title"])},\n'
            f'        "need_profit": {_py_lit(c["need_profit"])},\n'
            f'        "need_growth": {_py_lit(c["need_growth"])},\n'
            f'        "need_balance": {_py_lit(c["need_balance"])},\n'
            f'        "need_fin_db": {_py_lit(c["need_fin_db"])},\n'
            f'        "signal": {_signal_ref(c["signal"])},\n'
            f'        "params": _p(\n'
            + "\n".join(kw)
            + "\n        ),\n"
            f"    }},\n"
        ...
        blocks.append(block)
    if len(blocks) == 1:
        print("[registry] nothing new to patch", flush=True)
        return
    insert = "".join(blocks)
    # 插在 FACTOR_IMPL 字典闭合之前
    idx = text.index(marker)
    # 往前找到字典结束的 `}`
    before = text[:idx].rstrip()
    if not before.endswith("}"):
        raise SystemExit("ABORT: unexpected factor_registry ending before compute_factor_signal")
    # 最后一个 } 是 FACTOR_IMPL 的闭合
    last_brace = before.rfind("\n}")
    if last_brace < 0:
        raise SystemExit("ABORT: cannot find FACTOR_IMPL closing brace")
    new_text = before[:last_brace] + "\n" + insert + "\n}" + text[idx:]
    reg.write_text(new_text, encoding="utf-8")
    print(f"[registry] patched {len(blocks)-1} entries -> {reg}", flush=True)
...
def _desc(c: Dict[str, Any], params: Dict[str, Any], mine: dict) -> str:
    return (
        ...
        f"来源 mine_struct_mass_round2；静态成分幸存者偏差。"
    )
    ...
    for dbn, planned in plans.items():
        ...
        for fid, created_at, ui in planned:
            ...
            payload = {
                ...
                "mine_meta": {
                    "source": f"mine_struct_mass_round2/{c['univ']}",
                    "cfg_id": c["cfg_id"],
                    "family": c["family"],
...
def main() -> None:
    ...
    _patch_factor_registry(candidates, params_map)
    ...
    out = ROOT / "data" / "factors" / "fill_struct_mass_round2.json"
    out.write_text(
        json.dumps(
