...
echo "🔍 查找 柳暗花明 相关日志..."
docker logs mongodb-test 2>&1 | grep -i "lahm" || echo "⚠️  未找到 柳暗花明 相关日志"
echo ""
...
echo "--------------------------------------------------------------------------------"
if docker exec -it mongodb-test mongo -u admin -p lahm123 --authenticationDatabase admin --eval "db.version()" 2>/dev/null; then
    echo "✅ admin 用户认证成功"
    echo ""
    ...
    echo "📋 步骤 8: 查看用户列表"
    echo "--------------------------------------------------------------------------------"
    docker exec -it mongodb-test mongo -u admin -p lahm123 --authenticationDatabase admin --eval "
        use admin;
        print('=== Admin 数据库用户 ===');
...
try:
    # 测试连接
    uri = 'mongodb://admin:lahm123@localhost:27017/lahm?authSource=admin'
