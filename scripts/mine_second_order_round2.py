"""二阶挖掘 Round2：减速 / 领先滞后 / 营运资本二阶导 / 现金质量交叉。

相对 Round1（YoY 加速、因果 L2）的增量角度：
- 净利 YoY 软着陆减速（仍为正）
- 合同负债高位减速回踩；合同负债加速领先 × 净利尚未加速
- 存货/应收强度「改善的改善」（二阶导）
- 毛利率扩张二阶；销售收现比改善二阶；CFO/净利质量二阶
- 应收收紧 × 合同负债加速交叉；CFO YoY 再加速（物理族补挖）

- 主分 = 0.2×Sh(2018–21)+0.3×Sh(2022–23)+0.5×Sh(2024+)
- 去重：Mongo + 锚点 + mine_*；**绝不写 Mongo / 不注册 lahm**
- 产物：data/factors/mine_second_order_round2/
- 腾讯 qfq；BaoStock 禁用

用法:
  .venv\\Scripts\\python.exe scripts/mine_second_order_round2.py --skip-build
  .venv\\Scripts\\python.exe scripts/mine_second_order_round2.py --universes hs300 --skip-build
"""
...
OUT_ROOT = ROOT / "data" / "factors" / "mine_second_order_round2"
START = "2018-01-01"
...
def _grid() -> List[GridRow]:
    """Round2 新结构网格（避开 R1 已入库指纹）。"""
    rows: List[GridRow] = []

    # ---- 净利 YoY 软着陆减速 ----
    for tag, extra, ng in [
        (
            "soft_g10_d015_brk60_h40",
            _exit_pack(40, tp=0.28, lag=28, brk=60, entry="break", growth_min=0.10, decel_lo=-0.15, decel_hi=-0.02),
            True,
        ),
        (
            "soft_g12_d12_pull_h38_pe55",
            _exit_pack(
                38,
                tp=0.26,
                lag=30,
        ...
        rows.append(
            (f"ni_soft_decel__{tag}", "ni_yoy_soft_decel", sig.signal_ni_yoy_soft_decel_break, extra, True, ng, True, False)
        )

    # ---- 合同负债高位减速回踩 ----
    for tag, extra in [
        (
            "cldec_p20_d05_pull_h40",
            _exit_pack(40, tp=0.28, lag=28, brk=60, entry="pullback", prev_yoy_min=0.20, decel_max=-0.05, yoy_min=0.0, dd_need=0.03),
        ),
        (
            "cldec_p25_d08_pull_h42_np04",
            _exit_pack(
                42,
                tp=0.28,
                lag=30,
        ...
        rows.append(
            (f"cl_yoy_decel__{tag}", "cl_yoy_decel", sig.signal_cl_yoy_decel_reclaim, extra, True, False, True, True)
        )

    # ---- 合同负债领先、净利滞后 ----
    for tag, extra in [
        (
            "cllead08_y10_ni02_brk60_h40",
            _exit_pack(40, tp=0.30, lag=28, brk=60, entry="break", cl_accel=0.08, yoy_min=0.10, ni_accel_max=0.02, growth_min=-0.05),
        ),
        (
            "cllead12_y15_ni00_brk70_h45",
            _exit_pack(
                45,
                tp=0.32,
                lag=26,
        ...
        rows.append(
            (f"cl_lead_ni__{tag}", "cl_lead_ni_lag", sig.signal_cl_lead_ni_lag_break, extra, True, True, True, True)
        )

    # ---- 存货/应收强度改善二阶 ----
    for tag, extra in [
        (
            "invacc015_ry05_brk60_h40",
            _exit_pack(40, tp=0.30, lag=28, brk=60, entry="break", inv_accel=0.015, growth_min=0.05, inv_max=0.85),
        ),
        (
            "invacc02_ry08_brk55_h42_np04",
            _exit_pack(42, tp=0.30, lag=29, brk=55, entry="break", inv_accel=0.02, growth_min=0.08, inv_max=0.70, np_min=0.04),
        ),
    ]:
        rows.append(
            (f"inv_acc__{tag}", "inv_improve_accel", sig.signal_inv_improve_accel_break, extra, True, False, True, False)
        )

    for tag, extra in [
        (
            "aracc012_ry05_brk60_h40",
            _exit_pack(40, tp=0.30, lag=28, brk=60, entry="break", ar_accel=0.012, growth_min=0.05, ar_max=0.55),
        ),
        (
            "aracc018_ry08_brk70_h42",
            _exit_pack(42, tp=0.30, lag=26, brk=70, entry="break", ar_accel=0.018, growth_min=0.08, ar_max=0.45, np_min=0.04),
        ),
    ]:
        rows.append(
            (f"ar_acc__{tag}", "ar_improve_accel", sig.signal_ar_improve_accel_break, extra, True, False, True, False)
        )

    # ---- 毛利率扩张二阶 ----
    for tag, extra in [
        (
            "gpacc004_m14_ry05_brk60_h40",
            _exit_pack(40, tp=0.30, lag=28, brk=60, entry="break", gp_accel=0.004, margin_min=0.14, growth_min=0.05),
        ),
        (
            "gpacc006_m16_ry08_brk70_h45",
            _exit_pack(45, tp=0.32, lag=26, brk=70, entry="break", gp_accel=0.006, margin_min=0.16, growth_min=0.08, np_min=0.05),
        ),
    ]:
        rows.append(
            (f"gp_acc__{tag}", "gp_margin_accel", sig.signal_gp_margin_accel_break, extra, True, False, True, False)
        )

    # ---- 现金质量 / 收现二阶 / CFO YoY 加速 ----
    for tag, extra in [
        (
            "cfoq08_m06_g00_brk60_h40",
            _exit_pack(40, tp=0.30, lag=28, brk=60, entry="break", cfo_min=0.6, cfo_improve=0.08, growth_min=0.0),
        ),
        (
            "cfoq10_m08_g05_brk70_h42",
            _exit_pack(42, tp=0.30, lag=26, brk=70, entry="break", cfo_min=0.8, cfo_improve=0.10, growth_min=0.05, np_min=0.04),
        ),
    ]:
        rows.append(
            (
                f"cfo_np_acc__{tag}",
                "cfo_np_quality_accel",
                sig.signal_cfo_np_quality_accel_break,
                extra,
                True,
                True,
                True,
    ...
    for tag, extra in [
        (
            "ccacc02_cmin85_ry05_brk60_h40",
            _exit_pack(40, tp=0.30, lag=28, brk=60, entry="break", collect_accel=0.02, collect_min=0.85, growth_min=0.05),
        ),
        (
            "ccacc03_cmin90_ry08_brk70_h42",
            _exit_pack(42, tp=0.30, lag=26, brk=70, entry="break", collect_accel=0.03, collect_min=0.90, growth_min=0.08),
        ),
    ]:
        rows.append(
            (
                f"cash_acc__{tag}",
                "cash_collect_accel",
                sig.signal_cash_collect_accel_break,
                extra,
                True,
                False,
                True,
    ...
    for tag, extra in [
        (
            "cfoacc08_y05_brk60_h40",
            _exit_pack(40, tp=0.30, lag=28, brk=60, entry="break", cfo_accel=0.08, cfo_yoy_min=0.05),
        ),
        (
            "cfoacc12_y08_brk70_h45_np04",
            _exit_pack(45, tp=0.32, lag=26, brk=70, entry="break", cfo_accel=0.12, cfo_yoy_min=0.08, np_min=0.04),
        ),
    ]:
        rows.append(
            (f"cfo_yoy_acc__{tag}", "cfo_yoy_accel", sig.signal_cfo_yoy_accel_break, extra, True, False, True, False)
        )

    # ---- 应收收紧 × 合同负债加速 ----
    for tag, extra in [
        (
            "arcl_ar012_cl08_brk60_h40",
            _exit_pack(40, tp=0.30, lag=28, brk=60, entry="break", ar_improve=0.012, cl_accel=0.08, yoy_min=0.08),
        ),
        (
            "arcl_ar015_cl10_brk70_h42_np04",
            _exit_pack(
                42,
                tp=0.32,
                lag=26,
        ...
        rows.append(
            (f"ar_cl_dual__{tag}", "ar_cl_dual", sig.signal_ar_cl_dual_accel_break, extra, True, False, True, True)
        )
...
def mine_universe(
    ...
    print(f"\n======== mine second_order r2 {universe} ========", flush=True)
    base = _base(universe)
...
def _write_universe_md(universe: str, payload: Dict[str, Any]) -> Path:
    lines = [
        f"# Second-Order Round2 · {universe}",
        "",
...
def write_round_summary(all_univ: Dict[str, Dict[str, Any]], univ_meta: Dict[str, Any]) -> Path:
    lines = [
        "# 二阶挖掘 Round2（减速 / 领先滞后 / 营运资本二阶导 / 现金质量）",
        "",
        f"- 时间：{datetime.now().isoformat(timespec='seconds')}",
        "- 角度地图：`ANGLE_MAP.md`",
        "- 行情：腾讯前复权 `_shared/daily`；BaoStock 禁用",
        "- 成分：中证官网静态 xls（**幸存者偏差**；非 PIT）",
        "- 财务：本地财务库派生 + growth YoY",
        "- **Mongo / lahm：本轮绝不写入**（仅产物目录，待人工审阅）",
        "- 去重：`scripts/mine_factor_dedup.py`",
        "",
        "## 下载了什么",
        "",
        "- 未新下东财；profit cache 仅 fin_db `only_missing` 补洞",
        ...
        "| family | 含义 | signal |",
        "|--------|------|--------|",
        "| ni_yoy_soft_decel | 净利 YoY 软着陆减速 | `signal_ni_yoy_soft_decel_break` |",
        "| cl_yoy_decel | 合同负债高位减速回踩 | `signal_cl_yoy_decel_reclaim` |",
        "| cl_lead_ni_lag | 合同负债加速领先 × 净利未加速 | `signal_cl_lead_ni_lag_break` |",
        "| inv_improve_accel | 存货强度改善二阶 | `signal_inv_improve_accel_break` |",
        "| ar_improve_accel | 应收强度改善二阶 | `signal_ar_improve_accel_break` |",
        "| gp_margin_accel | 毛利率扩张二阶 | `signal_gp_margin_accel_break` |",
        "| cfo_np_quality_accel | CFO/净利质量二阶 | `signal_cfo_np_quality_accel_break` |",
        "| cash_collect_accel | 销售收现比改善二阶 | `signal_cash_collect_accel_break` |",
        "| cfo_yoy_accel | 经营现金流 YoY 再加速 | `signal_cfo_yoy_accel_break` |",
        "| ar_cl_dual | 应收收紧 × 合同负债加速 | `signal_ar_cl_dual_accel_break` |",
        "",
        "## 时间加权",
        "",
        "| 分段 | 区间 | 权重 |",
    ...
    lines.extend(
        [
            ...
            "- **本轮不入库**：结果仅在本目录；晚上人工审阅后再决定是否挂 lahm",
