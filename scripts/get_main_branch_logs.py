...
def find_lahm_container():
    """查找柳暗花明 Web容器"""
    print("🔍 查找柳暗花明 Web容器...")
    
    # 根据docker-compose.yml，容器名应该是 柳暗花明-web
    container_names = [
        "柳暗花明-web",
        "lahm-web", 
        "lahm_web_1",
...
def get_docker_logs(container_name):
    ...
    docker_log_file = f"lahm_docker_logs_{timestamp}.log"
...
import logging
print("Python日志配置:")
print(f"  日志级别: {os.getenv('LAHM_LOG_LEVEL', 'NOT_SET')}")
print(f"  日志目录: {os.getenv('LAHM_LOG_DIR', 'NOT_SET')}")
...
def main():
    """主函数"""
    print("🚀 柳暗花明 主分支日志获取工具")
    print("=" * 60)
    print(f"⏰ 执行时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # 1. 查找容器
    container_name = find_lahm_container()
    if not container_name:
        print("❌ 未找到柳暗花明容器，请确保容器正在运行")
        print("\n💡 启动容器的命令:")
