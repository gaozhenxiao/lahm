...
build_backend() {
    ...
    docker build \
        ...
        -t lahm-backend:latest \
        -t lahm-backend:pdf-support \
        .
...
test_pdf_export() {
    ...
    # 检查 WeasyPrint
    docker exec lahm-pdf-test python -c "
import sys
try:
    import weasyprint
...
" || print_warning "pdfkit 检测失败"
    ...
    # 检查 Pandoc
    docker exec lahm-pdf-test pandoc --version | head -n 1 || print_warning "Pandoc 检测失败"
    
    # 检查 wkhtmltopdf
    docker exec lahm-pdf-test wkhtmltopdf --version || print_warning "wkhtmltopdf 检测失败"
    
    # 停止测试容器
    print_info "停止测试容器..."
    docker stop lahm-pdf-test
...
show_usage() {
    cat << EOF
${GREEN}柳暗花明 Docker 构建脚本（PDF 导出支持）${NC}
...
${GREEN}下一步:${NC}
1. 测试 PDF 导出功能:
   docker run --rm -p 8000:8000 lahm-backend:latest
