#!/bin/bash
# 柳暗花明 AMD64 (x86_64) 架构 Docker 镜像构建脚本
...
# 镜像名称（AMD64 独立仓库）
BACKEND_IMAGE="lahm-backend-amd64"
FRONTEND_IMAGE="lahm-frontend-amd64"

# 目标架构
PLATFORM="linux/amd64"

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}柳暗花明 AMD64 镜像构建${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
BUILDER_NAME="lahm-builder-amd64"
...
if [ -n "$REGISTRY" ]; then
    ...
    echo -e "  - ARM64 版本使用独立仓库: ${REGISTRY}/lahm-backend-arm64"
    echo -e "  - 可以独立更新，互不影响"
else
...
echo -e "   docker images | grep lahm"
echo ""
