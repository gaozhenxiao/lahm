...
Write-Host "   - lahm_mongodb_data (MongoDB 数据)" -ForegroundColor White
Write-Host "   - lahm_redis_data (Redis 数据)" -ForegroundColor White
Write-Host ""
Write-Host "🗑️  将要删除的容器:" -ForegroundColor Yellow
Write-Host "   - lahm-mongodb" -ForegroundColor White
Write-Host "   - lahm-redis" -ForegroundColor White
Write-Host "   - lahm-backend" -ForegroundColor White
Write-Host "   - lahm-frontend" -ForegroundColor White
Write-Host "   - lahm-nginx (如果存在)" -ForegroundColor White
Write-Host ""
...
docker volume ls --format "table {{.Name}}\t{{.Driver}}\t{{.Scope}}" | Select-String "lahm"
