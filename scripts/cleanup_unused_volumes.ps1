...
# 3. 列出所有 柳暗花明 相关的数据卷
Write-Host "`n📊 柳暗花明 相关的数据卷:" -ForegroundColor Yellow

$allVolumes = docker volume ls --format "{{.Name}}" | Where-Object { 
    $_ -like "*lahm*" -or $_ -like "*mongodb*" -or $_ -like "*redis*"
}
...
Write-Host "  - lahm_mongodb_data (主数据卷)" -ForegroundColor Cyan
Write-Host "  - lahm_redis_data (主数据卷)" -ForegroundColor Cyan
