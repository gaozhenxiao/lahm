...
echo "🚀 柳暗花明 Docker日志获取工具"
echo "=================================="

# 查找容器
CONTAINER_NAMES=("lahm-data-service" "lahm_data-service_1" "data-service" "lahm-data-service-1")
CONTAINER=""
...
echo ""
echo "💡 使用建议:"
echo "   - 如果源码目录的lahm.log为空，说明日志可能输出到stdout"
echo "   - Docker标准日志包含了应用的所有输出"
echo "   - 检查应用的日志配置，确保日志写入到文件"
echo ""
echo "📧 发送日志文件:"
echo "   请将 docker_logs_${TIMESTAMP}.log 文件发送给开发者"
if [ -f "lahm.log_${TIMESTAMP}" ]; then
    echo "   以及 lahm.log_${TIMESTAMP} 文件"
fi
