#!/bin/bash
# 柳暗花明 Docker镜像构建和发布脚本（Linux服务器版 - 多架构支持）
...
echo -e "${CYAN}========================================${NC}"
echo -e "${CYAN}柳暗花明 Docker多架构构建和发布${NC}"
echo -e "${CYAN}========================================${NC}"
echo ""
...
# 配置
BACKEND_IMAGE_REMOTE="$DOCKERHUB_USERNAME/lahm-backend"
FRONTEND_IMAGE_REMOTE="$DOCKERHUB_USERNAME/lahm-frontend"
BUILDER_NAME="lahm-builder"
...
docker images | grep "lahm-backend" | grep "$VERSION" | awk '{print $3}' | xargs -r docker rmi -f 2>/dev/null || true
docker images | grep "lahm-frontend" | grep "$VERSION" | awk '{print $3}' | xargs -r docker rmi -f 2>/dev/null || true
docker images | grep "$DOCKERHUB_USERNAME/lahm-backend" | awk '{print $3}' | xargs -r docker rmi -f 2>/dev/null || true
docker images | grep "$DOCKERHUB_USERNAME/lahm-frontend" | awk '{print $3}' | xargs -r docker rmi -f 2>/dev/null || true
