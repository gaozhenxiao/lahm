...
Write-Host "  1. 重启后端服务: docker restart lahm-backend" -ForegroundColor Cyan
...
``````powershell
# 检查系统配置
docker exec lahm-mongodb mongo lahm -u admin -p lahm123 --authenticationDatabase admin --eval "db.system_configs.countDocuments()"

# 检查用户数量
docker exec lahm-mongodb mongo lahm -u admin -p lahm123 --authenticationDatabase admin --eval "db.users.countDocuments()"
``````
"@
