...
if docker exec lahm-mongodb mongo --eval "db.adminCommand('ping')" > /dev/null 2>&1; then
...
# 检查后端容器是否运行
if ! docker ps | grep -q "lahm-backend"; then
...
if docker exec lahm-backend test -f scripts/quick_login_fix.py; then
    docker exec lahm-backend python scripts/quick_login_fix.py
...
if docker exec lahm-backend test -f scripts/simple_auth_migration.py; then
    docker exec lahm-backend python scripts/simple_auth_migration.py
