...
class LoggingMigrator:
    ...
    def __init__(self, project_root: Path):
        ...
        self.module_logger_map = {
            ...
            'lahm/llm_adapters': 'setup_llm_logging',
            'lahm/dataflows': 'setup_dataflow_logging',
            'lahm/graph': 'get_logger("graph")',
            'lahm/agents': 'get_logger("agents")',
            'lahm/api': 'get_logger("api")',
            'lahm/utils': 'get_logger("utils")',
            'cli': 'get_logger("cli")',
    ...
    def _add_logging_import(self, content: str, file_path: Path) -> str:
        """添加统一日志导入"""
        # 检查是否已经有统一日志导入
        if 'from lahm.utils.logging_init import' in content:
            return content
        ...
        if logger_func.startswith('setup_'):
            import_line = f"from lahm.utils.logging_init import {logger_func}"
            logger_line = f"logger = {logger_func}()"
        else:
            import_line = f"from lahm.utils.logging_init import get_logger"
            logger_line = f"logger = {logger_func}"
