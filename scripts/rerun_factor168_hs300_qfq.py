"""腾讯前复权日线就绪后，wave84 同参重跑因子168（静态 HS300），仅写本地，不写 Mongo。"""
...
FACTOR_ID = "gross_expand_m16_tp35_hs300_qfq_rerun"
OUT = ROOT / "data" / "factors" / "gross_expand_m16_tp35_hs300_qfq_rerun.json"

# wave84 / overnight_keep：signal_gross_expand_break；无 net_profit_min
PARAMS = {
    "universe": "hs300",
...
POLLUTED = {"total_return": -0.28, "sharpe": -0.08}
...
def main() -> None:
    ...
    summary = runner.run_factor_pipeline(
        FACTOR_ID,
        "qfq rerun wave84 gross_expand_m16_tp35 static hs300",
    ...
    payload = {
        ...
        "source": "wave84_tp35_cross / overnight_keep + tencent qfq daily",
        "note": "local _shared/daily qfq; BaoStock off; no Mongo",
        "factor_id_original": "gross_expand_m16_tp35",
        "signal": "signal_gross_expand_break",
        "universe": "hs300",
        "baostock": "disabled_cache_only",
        "mongo_written": False,
        "target_wave84": TARGET,
        "polluted_baseline": POLLUTED,
        "reproduced_wave84": ok,
        "params": PARAMS,
        "summary": summary,
    }
    ...
    print(
        ...
        f"(wave84 {TARGET['total_return']}/{TARGET['sharpe']}; "
        f"polluted {POLLUTED['total_return']}/{POLLUTED['sharpe']})",
        flush=True,
    )
