...
def write_md(rows, path: Path, title: str) -> None:
    md = [
        title,
        "",
        "成本：佣金万一 + 印花税千一。思路偏另类（流动性/波动体制/日历/拥挤度等）。",
        "",
...
def main() -> None:
    ...
    run(
        [
            ...
            only,
            "--out",
            "data/factors/wave4_smoke_summary.json",
        ]
    )
    smoke = rank_rows(ROOT / "data/factors/wave4_smoke_summary.json")
    ...
    run(
        [
            ...
            ",".join(keep),
            "--out",
            "data/factors/wave4_full_summary.json",
        ]
    )
    # plots
    try:
    ...
    rows = rank_rows(ROOT / "data/factors/wave4_full_summary.json")
    write_md(rows, ROOT / "data/factors/wave4_rank.md", "# 第四波另类因子全量回测（沪深300，2018–2026）")
    ...
    # smoke rejects also weak
    smoke_data = json.loads((ROOT / "data/factors/wave4_smoke_summary.json").read_text(encoding="utf-8"))
    ...
    (ROOT / "data/factors/wave4_keep.json").write_text(
