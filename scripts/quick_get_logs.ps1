# 柳暗花明 Docker日志获取工具 (PowerShell版本)

Write-Host "🚀 柳暗花明 Docker日志获取工具" -ForegroundColor Green
Write-Host "==================================" -ForegroundColor Green

# 查找容器
$ContainerNames = @("lahm-data-service", "lahm_data-service_1", "data-service", "lahm-data-service-1")
$Container = $null
...
if (-not $Container) {
    Write-Host "❌ 未找到柳暗花明容器" -ForegroundColor Red
...
Write-Host "   - 如果源码目录的lahm.log为空，说明日志可能输出到stdout" -ForegroundColor Gray
...
if (Test-Path "lahm.log_$Timestamp") {
    Write-Host "   以及 lahm.log_$Timestamp 文件" -ForegroundColor Gray
}
