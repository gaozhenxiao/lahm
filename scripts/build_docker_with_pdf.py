...
def test_docker_container():
    ...
    # 启动容器进行测试
    start_cmd = """docker run -d --name lahm-test \
    ...
    # 获取测试结果
    logs_cmd = "docker logs lahm-test"
    result = run_command(logs_cmd, "获取测试日志", timeout=30)
    
    # 清理测试容器
    cleanup_cmd = "docker rm -f lahm-test"
...
def main():
    ...
    logger.info(f"\n2. 仅启动Web服务:")
    logger.info(f"   docker run -p 8501:8501 lahm:latest")
    logger.info(f"\n3. 测试PDF功能:")
    logger.info(f"   docker run lahm:latest python scripts/test_docker_pdf.py")
