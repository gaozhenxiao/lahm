...
def check_old_volume():
    ...
    # 6. 查看 lahm 数据库的集合
    print(f"\n6️⃣ 查看 lahm 数据库的集合...")
    cmd = f"docker exec {temp_container} mongosh lahm --quiet --eval 'db.getCollectionNames()'"
    code, stdout, stderr = run_command(cmd, shell=True)
    ...
    cmd = f"""docker exec {temp_container} mongosh lahm --quiet --eval '
    ...
    for coll in collections:
        cmd = f"docker exec {temp_container} mongosh lahm --quiet --eval 'db.{coll}.countDocuments()'"
        code, stdout, stderr = run_command(cmd, shell=True)
        if code == 0:
            count = stdout.strip()
    ...
    print(f"\n🔧 您可以使用以下命令连接到旧数据库:")
    print(f"  mongosh mongodb://localhost:27018/lahm")
    
    print(f"\n🔧 或使用 MongoDB Compass 连接:")
    print(f"  连接字符串: mongodb://localhost:27018/lahm")
