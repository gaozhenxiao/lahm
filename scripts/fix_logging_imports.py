...
class LoggingImportFixer:
    ...
    def fix_logging_import_position(self, content: str, file_path: Path) -> str:
        ...
        for i, line in enumerate(lines):
            if ('# 导入日志模块' in line or 
                'from lahm.utils.logging_manager import get_logger' in line or 
        ...
        if 'web' in str(relative_path):
            logger_name = 'web'
        elif 'lahm' in str(relative_path):
        ...
        lines.insert(insert_pos + 2, "from lahm.utils.logging_manager import get_logger")
    ...
    def fix_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            # 检查是否包含日志导入
            if 'from lahm.utils.logging_manager import get_logger' not in content:
                return False
