...
$mongoContainer = docker ps -a --filter "name=lahm-mongodb" --format "{{.Names}}"
$redisContainer = docker ps -a --filter "name=lahm-redis" --format "{{.Names}}"
...
Write-Host "  目标 MongoDB 数据卷: lahm_mongodb_data" -ForegroundColor Cyan
Write-Host "  目标 Redis 数据卷: lahm_redis_data" -ForegroundColor Cyan
...
$network = docker network ls --filter name=lahm-network --format "{{.Name}}"
if (-not $network) {
    Write-Host "  创建网络..." -ForegroundColor Yellow
    docker network create lahm-network | Out-Null
...
docker run -d `
  ...
  --network lahm-network `
  -p 6379:6379 `
  -v lahm_redis_data:/data `
  -e TZ="Asia/Shanghai" `
  --restart unless-stopped `
  redis:7-alpine redis-server --appendonly yes --requirepass lahm123 | Out-Null
...
$mongoVolume = docker inspect lahm-mongodb -f '{{range .Mounts}}{{if eq .Destination "/data/db"}}{{.Name}}{{end}}{{end}}'
$redisVolume = docker inspect lahm-redis -f '{{range .Mounts}}{{if eq .Destination "/data"}}{{.Name}}{{end}}{{end}}'

Write-Host "  MongoDB 数据卷: $mongoVolume" -ForegroundColor Cyan
Write-Host "  Redis 数据卷: $redisVolume" -ForegroundColor Cyan

if ($mongoVolume -eq "lahm_mongodb_data" -and $redisVolume -eq "lahm_redis_data") {
...
$dbCheck = docker exec lahm-mongodb mongo lahm -u admin -p lahm123 --authenticationDatabase admin --quiet --eval "db.system_configs.countDocuments()" 2>$null
...
docker ps --filter "name=lahm" --format "table {{.Names}}`t{{.Status}}`t{{.Ports}}"

Write-Host "`n  数据卷列表:" -ForegroundColor Cyan
docker volume ls --filter "name=lahm"
...
Write-Host "  2. 检查 MongoDB 数据: docker exec lahm-mongodb mongo lahm -u admin -p lahm123 --authenticationDatabase admin" -ForegroundColor Cyan
Write-Host "  3. 重启后端服务（如果需要）: docker restart lahm-backend" -ForegroundColor Cyan
