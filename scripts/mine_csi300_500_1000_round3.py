"""三宇宙 Round3：在 Round2 novel 基础上开新网格/新结构。

- 主分 = 0.2×Sharpe(2018–21) + 0.3×Sharpe(2022–23) + 0.5×Sharpe(2024+)
- 近2年(2024-08+)大幅亏损且远年虚高 → 降权/剔除（#171 教训）
- 去重：Mongo + 锚点#166/#168/#171 + #173–#183 + Round2 全部指纹
- 不写 Mongo；产物 data/factors/mine_csi300_500_1000_round3/
- 静态成分有幸存者偏差（非 PIT）

用法:
  .venv\\Scripts\\python.exe scripts/mine_csi300_500_1000_round3.py --skip-build
  .venv\\Scripts\\python.exe scripts/mine_csi300_500_1000_round3.py --universes hs300 --skip-build
"""
...
OUT_ROOT = ROOT / "data" / "factors" / "mine_csi300_500_1000_round3"
START = "2018-01-01"
...
# Round3 新分档边（尚未被 round1/round2 fmkv 用过）：400亿 / 1500亿
EDGES_YUAN_C = [4e10, 1.5e11]
# 另一组：200亿 / 800亿（偏小盘友好）
EDGES_YUAN_D = [2e10, 8e10]
...
def _grid() -> List[GridRow]:
    """Round3 尚未覆盖：新结构族 + 新 f(mkv) 边 C/D + entry/过滤器变体。

    返回 (cfg_id, family, fn, extras, need_profit, need_growth, need_fin_db, need_balance)。
    Round2 / #166–#185 由 DedupIndex 在运行时 skip。
    """
    rows: List[GridRow] = []
    ge = sig.signal_gross_expand_break

    # ---- 新 f(mkv) 分档边 C/D（区别于 round1 5e10/2e11 与 round2 3e10/1.2e11）----
    for tag, extra in [
        (
            "fmkv_c_edges_mbrk",
            dict(
                ...
                stop_loss=0.12,
                take_profit=0.32,
                margin_min_by_mkv={"edges": EDGES_YUAN_C, "values": [0.12, 0.15, 0.18]},
                break_days_by_mkv={"edges": EDGES_YUAN_C, "values": [80, 55, 40]},
            ),
        ),
        (
            "fmkv_d_soft_small",
            dict(
                ...
                stop_loss=0.12,
                take_profit=0.28,
                margin_min_by_mkv={"edges": EDGES_YUAN_D, "values": [0.11, 0.14, 0.17]},
                break_days_by_mkv={"edges": EDGES_YUAN_D, "values": [100, 70, 50]},
            ),
        ),
        (
            "fmkv_c_npmin5e8",
            dict(
                ...
                take_profit=0.33,
                net_profit_min=5e8,
                margin_min_by_mkv={"edges": EDGES_YUAN_C, "values": [0.14, 0.17, 0.20]},
                break_days=65,
            ),
        ),
        (
        ...
        rows.append((f"ge_r3__{tag}", "gross_expand_r3", ge, extra, True, False, False, False))

    # ---- 新信号结构（Round2 未覆盖的主线）----
    for tag, fn, extra, nb, ng in [
        (
            "gp_consec_m15_lag27",
            sig.signal_gp_consec_break,
            dict(
        ...
        (
            "asset_light_cl_lag28",
            sig.signal_asset_light_cl_break,
            dict(
                qoq_min=0.08,
                yoy_min=0.15,
                asset_yoy_max=0.15,
        ...
        (
            "parent_eps_twin_g15",
            sig.signal_parent_eps_twin_break,
            dict(
                growth_min=0.15,
                roe_min=0.08,
                pe_pct_max=0.55,
        ...
        rows.append((f"struct_r3__{tag}", "struct_r3", fn, extra, True, ng, False, nb))

    # ---- q_np 新变体（区别 Round2 exp120/exp60）----
    ...
    for tag, extra in [
        ...
        rows.append((f"q_np_r3__{tag}", "q_np_gap_r3", qg, extra, False, False, True, False))
    ...
    for tag, extra in [
        (
            ...
            dict(
                ...
                take_profit=0.25,
                mktcap_min=3e9,
                margin_min_by_mkv={"edges": EDGES_YUAN_D, "values": [0.10, 0.13, 0.16]},
                break_days_by_mkv={"edges": EDGES_YUAN_D, "values": [90, 65, 45]},
            ),
        ),
        (
            "m12_brk85_h40_yoy10",
        ...
        rows.append((f"ge_mid_r3__{tag}", "gross_expand_mid_r3", ge, extra, True, False, False, False))
...
def mine_universe(
    ...
    print(f"\n======== mine round3 {universe} ========", flush=True)
    base = _base(universe)
...
def write_round_summary(all_univ: Dict[str, Dict[str, Any]]) -> Path:
    lines = [
        "# 三宇宙分挖因子 · Round3（时间加权 + 去重；新结构/新 fmkv）",
        "",
    ...
    lines.append("")
    lines.append("## 说明")
    lines.append("- Round1 主看全样本 Sharpe；Round2/3 主看时间加权 + 近2年")
    lines.append("- 本轮相对 Round2：新结构（gp_consec/np_regime/cl_intensity/asset_light/rev_qoq/…）+ fmkv 边 C/D")
    lines.append("- dual_improve meanrev / #166/#168/#171 锚点 / #173–#185 已挂不作为新发现")
    lines.append("- 入库前：`DedupIndex.check` + `max(序号)+1`（当前 Mongo max_ui=185 → 下一号 ≥186）；宁少勿滥")
    lines.append("- 成分：静态 xls **幸存者偏差**（非 PIT）")

    path = OUT_ROOT / "SUMMARY.md"
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    (OUT_ROOT / "round3_index.json").write_text(
        json.dumps(
            {
