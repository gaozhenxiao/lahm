...
# ============================================================================
# Features:
# 1. Sync core files from main codebase to release/Lahm-portable
...
$portableDir = Join-Path $root "release\Lahm-portable"
...
if (-not $SkipDependencies) {
    ...
    if ((Test-Path $sourceReq) -and (Test-Path $destReq)) {
        ...
        if ($sourceHash -ne $destHash) {
            ...
            Write-Host "  Command: cd release\Lahm-portable; venv\Scripts\pip install -r requirements.txt" -ForegroundColor Yellow
        } else {
...
Write-Host "  1. Update dependencies: cd release\Lahm-portable; venv\Scripts\pip install -r requirements.txt" -ForegroundColor White
Write-Host "  2. Test portable version: cd release\Lahm-portable; powershell -ExecutionPolicy Bypass -File start_all.ps1" -ForegroundColor White
