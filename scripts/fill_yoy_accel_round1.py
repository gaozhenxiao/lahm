"""把 YoY 增速趋势 Round1 Top 候选写入 lahm（仅 INSERT）。

来源：data/factors/mine_yoy_accel_round1/{univ}/results.json
行情：腾讯 qfq（_shared/daily）；BaoStock 禁用。
硬性：不 update 已有 factor_id；不填空洞；撞号 abort。
MIN_UI=197（主库已至 #196；不覆盖 #188）。
宁少勿滥：仅入库 HS300 三条扎实候选。
"""
...
MINE_ROOT = ROOT / "data" / "factors" / "mine_yoy_accel_round1"
MIN_UI = 197
PROTECTED = (
...
CANDIDATES: List[Dict[str, Any]] = [
    {
        "factor_id": "dual_rev_np_yoy_dual04_hs300_yoy",
        "cfg_id": "dual_rev_np_yoy__dual04_g06_brk60_h42",
        "univ": "hs300",
        "signal": sig.signal_dual_rev_np_yoy_accel_break,
        "signal_name": "signal_dual_rev_np_yoy_accel_break",
        "need_profit": True,
        "need_growth": True,
        "need_balance": False,
        "need_fin_db": True,
    ...
    {
        "factor_id": "rev_yoy_acc_racc05_hs300_yoy",
        "cfg_id": "rev_yoy_acc__racc05_g08_brk60_h40",
        "univ": "hs300",
        "signal": sig.signal_rev_yoy_accel_break,
        "signal_name": "signal_rev_yoy_accel_break",
        "need_profit": True,
        "need_growth": False,
        "need_balance": False,
        "need_fin_db": True,
    ...
    {
        "factor_id": "q_np_yoy_acc_qacc15_hs300_yoy",
        "cfg_id": "q_np_yoy_acc__qacc15_g20_brk80_h35",
        "univ": "hs300",
        "signal": sig.signal_q_np_yoy_accel_break,
        "signal_name": "signal_q_np_yoy_accel_break",
        "need_profit": False,
        "need_growth": False,
        "need_balance": False,
        "need_fin_db": True,
...
def _desc(c: dict, params: dict, mine: dict) -> str:
    return (
        f"mine_yoy_accel_round1 / {c['univ']} `{c['cfg_id']}`"
        f"（时间加权 tw≈{mine.get('tw_score'):.3f}；YoY 增速趋势 R1）。"
...
def _insert_mongo(summaries: Dict[str, dict], params_map: Dict[str, dict], mine_map: Dict[str, dict]) -> None:
    ...
    for dbn, planned in plans.items():
        ...
        # 仅确认锚点仍存在；UI 序号可能因 #192 pad 等占位偏移，不做硬编码对位
        for pf in (
            "gross_expand_lag28_tp35",
            "gross_expand_m16_tp35",
            "dual_improve_hs300_mine_r1",
            ...
            if got is None:
                raise SystemExit(f"ABORT {dbn}: protected factor missing before insert: {pf}")
        ...
        for fid, created_at, ui in planned:
            ...
            payload = {
                ...
                "mine_meta": {
                    "source": f"mine_yoy_accel_round1/{c['univ']}",
                    "cfg_id": c["cfg_id"],
...
def main() -> None:
    ...
    for univ in sorted({c["univ"] for c in CANDIDATES}):
        ...
        print(
            f"[panel] preparing {univ} profit={need_profit} growth={need_growth} fin_db={need_fin_db}...",
            flush=True,
        )
    ...
    out = ROOT / "data" / "factors" / "fill_yoy_accel_round1.json"
    payload = {
    ...
    if summary_md.exists():
        ...
        extra.extend(
            [
                "",
                "未挂：CSI500/CSI1000（growth 覆盖稀疏或 full_sh 偏弱）；"
                "HS300 turn/ni_yoy_acc（近窗或全样本弱于上列）。",
                "",
            ]
        )
