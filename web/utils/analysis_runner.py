...
# 导入日志模块
from lahm.utils.logging_manager import get_logger, get_logger_manager
...
# 导入统一日志系统
from lahm.utils.logging_init import setup_web_logging
logger = setup_web_logging()

# 添加配置管理器
try:
    from lahm.config.config_manager import token_tracker
...
def run_stock_analysis(stock_symbol, analysis_date, analysts, research_depth, llm_provider, llm_model, market_type="美股", progress_callback=None):
    ...
    try:
        ...
        if not config.get("data_dir") or config["data_dir"] == "./data":
            env_data_dir = os.getenv("LAHM_DATA_DIR")
            if env_data_dir:
        ...
        if not config.get("results_dir") or config["results_dir"] == "./results":
            env_results_dir = os.getenv("LAHM_RESULTS_DIR")
            if env_results_dir:
                # 如果环境变量是相对路径，相对于项目根目录解析
        ...
        if not config.get("data_cache_dir"):
            env_cache_dir = os.getenv("LAHM_CACHE_DIR")
            if env_cache_dir:
                # 如果环境变量是相对路径，相对于项目根目录解析
                ...
                config["data_cache_dir"] = str(project_root / "lahm" / "dataflows" / "data_cache")
        ...
        logger.info(f"  - 环境变量 LAHM_RESULTS_DIR: {os.getenv('LAHM_RESULTS_DIR', '未设置')}")
        ...
        graph = LahmGraph(analysts, config=config, debug=False)
