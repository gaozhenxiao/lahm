...
# 旧品牌环境变量兼容：TRADINGAGENTS_* -> LAHM_*
for _k, _v in list(os.environ.items()):
    if _k.startswith("TRADINGAGENTS_"):
        _nk = "LAHM_" + _k[len("TRADINGAGENTS_") :]
        if _nk not in os.environ:
            os.environ[_nk] = _v
# 旧库名兼容：若未显式配置新库名且仍使用旧默认库，可在 .env 中设置 MONGODB_DATABASE=lahm
if "MONGODB_DATABASE" not in os.environ and os.environ.get("MONGODB_DATABASE_NAME") in {
    "tradingagentscn",
    "tradingagents",
}:
    # 不自动改写连接目标，避免误切库；仅保留别名提示由运维迁移
    pass
...
class Settings(BaseSettings):
    ...
    MONGODB_DATABASE: str = Field(default="lahm")
    ...
    LOG_FILE: str = Field(default="logs/lahm.log")
    ...
    NATIONAL_TEAM_FACTOR_REFRESH_ON_STARTUP: bool = Field(
        default=True,
        description="启动后异步计算一次 national_team 点信号",
    )
    NATIONAL_TEAM_FACTOR_REFRESH_ENABLED: bool = Field(
        default=True,
        description="是否注册 national_team 定时刷新任务",
    )
    ...
    # 数据目录配置
    LAHM_DATA_DIR: str = Field(default="./data")
...
def _read_major_version() -> str:
    v = os.getenv("LAHM_VERSION", "").strip() or os.getenv("APP_VERSION", "").strip()
    if not v:
        try:
...
def _default_instance_tag() -> str:
    user = os.getenv("LAHM_DB_USER", "").strip() or getpass.getuser()
    host = os.getenv("LAHM_DB_HOST", "").strip() or os.getenv("COMPUTERNAME", "").strip() or os.getenv("HOSTNAME", "").strip()
