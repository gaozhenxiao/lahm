"""
配置桥接模块
将统一配置系统的配置桥接到环境变量，供 柳暗花明 核心库使用
"""
...
def bridge_config_to_env():
    ...
    try:
        ...
        try:
            ...
            if use_mongodb.lower() == "true":
                try:
                    ...
                    if config_manager.mongodb_storage.is_connected():
                        logger.info("✅ lahm MongoDB 存储已启用")
                    else:
                        logger.warning("⚠️ lahm MongoDB 连接失败，将使用 JSON 文件存储")
            ...
            logger.error(f"❌ 重新初始化 lahm MongoDB 存储失败: {e}")
            import traceback
            logger.error(traceback.format_exc())

        # 7. 同步定价配置到 lahm 的 config/pricing.json
...
def get_bridged_model(model_type: str = "default") -> Optional[str]:
    ...
    """
    if model_type == "quick":
        return os.environ.get('LAHM_QUICK_MODEL')
    elif model_type == "deep":
        return os.environ.get('LAHM_DEEP_MODEL')
    else:
        return os.environ.get('LAHM_DEFAULT_MODEL')
...
def _sync_pricing_config(llm_configs):
    """
    同步定价配置到 lahm 的 config/pricing.json
