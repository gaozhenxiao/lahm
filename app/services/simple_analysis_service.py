...
# 初始化柳暗花明日志系统
from lahm.utils.logging_init import init_logging
init_logging()
...
# 股票基础信息获取（用于补充显示名称）
try:
    from lahm.dataflows.data_source_manager import get_data_source_manager
...
def get_provider_and_url_by_model_sync(model_name: str) -> dict:
    ...
    try:
        ...
        if doc and "llm_configs" in doc:
            ...
            for config_dict in llm_configs:
                if config_dict.get("model_name") == model_name:
                    ...
                    from lahm.llm_clients.provider_keys import normalize_provider_key, default_backend_url
        ...
        from lahm.llm_clients.provider_keys import normalize_provider_key
...
def _get_env_api_key_for_provider(provider: str) -> str:
    ...
    from lahm.llm_clients.provider_keys import env_key_for_provider, normalize_provider_key
...
def _get_default_backend_url(provider: str) -> str:
    ...
    Returns:
        str: 默认的 backend_url
    """
    from lahm.llm_clients.provider_keys import default_backend_url, normalize_provider_key
...
class SimpleAnalysisService:
    ...
    def _get_trading_graph(self, config: Dict[str, Any]) -> LahmGraph:
        """获取或创建柳暗花明实例

        ⚠️ 注意：为了避免并发执行时的数据混淆，每次都创建新实例
        虽然这会增加一些初始化开销，但可以确保线程安全

        LahmGraph 实例包含可变状态（self.ticker, self.curr_state等），
        如果多个线程共享同一个实例，会导致数据混淆。
        """
        # 🔧 [并发安全] 每次都创建新实例，避免多线程共享状态
        # 不再使用缓存，因为 LahmGraph 有可变的实例变量
        ...
        logger.info(f"✅ 柳暗花明实例创建成功（实例ID: {id(trading_graph)}）")
    ...
    def _run_analysis_sync(
        ...
        try:
            ...
            from lahm.utils.dataflow_utils import get_trading_date_range
    ...
    async def _save_modular_reports_to_data_dir(self, result: Dict[str, Any], stock_symbol: str) -> Dict[str, str]:
        ...
        try:
            ...
            results_dir_env = os.getenv("LAHM_RESULTS_DIR")
            if results_dir_env:
