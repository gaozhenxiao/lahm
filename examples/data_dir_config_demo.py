...
from lahm.dataflows.config import get_config, set_data_dir, get_data_dir
...
def demo_set_custom_data_dir():
    ...
    # 设置自定义数据目录
    custom_data_dir = os.path.join(os.path.expanduser("~"), "Documents", "Lahm_Custom", "data")
...
def demo_environment_variable_override():
    ...
    # 模拟设置环境变量
    test_env_dir = os.path.join(os.path.expanduser("~"), "Documents", "Lahm_ENV", "data")
    os.environ["LAHM_DATA_DIR"] = test_env_dir
    
    logger.info(f"设置环境变量 LAHM_DATA_DIR = {test_env_dir}")
...
def demo_directory_auto_creation():
    ...
    # 设置一个新的数据目录
    test_dir = os.path.join(os.path.expanduser("~"), "Documents", "Lahm_AutoCreate", "data")
...
```python
from lahm.dataflows.config import set_data_dir
...
```bash
# Windows
set LAHM_DATA_DIR=C:\\path\\to\\data

# Linux/Mac
export LAHM_DATA_DIR=/path/to/data
```
...
def main():
    """主演示函数"""
    logger.info(f"[bold blue]🎯 柳暗花明 数据目录配置演示[/bold blue]")
    logger.info(f"=")
