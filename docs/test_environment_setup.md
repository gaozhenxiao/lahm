...
本文档介绍如何使用独立的测试环境来验证 柳暗花明 的部署，而不影响现有的生产数据。
...
| **容器名称** | `lahm-*` | `lahm-*-test` |
| **数据卷名称** | `lahm_mongodb_data`<br>`lahm_redis_data` | `lahm_test_mongodb_data`<br>`lahm_test_redis_data` |
| **网络名称** | `lahm-network` | `lahm-test-network` |
| **日志目录** | `logs/` | `logs-test/` |
...
[INFO] Test data volumes:
  - lahm_test_mongodb_data
  - lahm_test_redis_data
...
xxxxxxxxxx     hsliup/lahm-frontend:latest   Up 2 minutes   0.0.0.0:3000->80/tcp       lahm-frontend-test
xxxxxxxxxx     hsliup/lahm-backend:latest    Up 2 minutes   0.0.0.0:8000->8000/tcp     lahm-backend-test
xxxxxxxxxx     redis:7-alpine                         Up 2 minutes   0.0.0.0:6379->6379/tcp     lahm-redis-test
xxxxxxxxxx     mongo:4.4                              Up 2 minutes   0.0.0.0:27017->27017/tcp   lahm-mongodb-test
```
...
**预期输出**：
```
local     lahm_mongodb_data           # 生产数据卷（保留）
local     lahm_redis_data             # 生产数据卷（保留）
local     lahm_test_mongodb_data      # 测试数据卷（新建）
local     lahm_test_redis_data        # 测试数据卷（新建）
```

#### 查看后端日志

```powershell
docker logs -f lahm-backend-test
```
...
**A**: 不会。测试环境使用独立的数据卷（`lahm_test_*`），与生产数据卷（`lahm_*`）完全隔离。
...
# 前端日志
docker logs -f lahm-frontend-test

# MongoDB 日志
docker logs -f lahm-mongodb-test

# Redis 日志
docker logs -f lahm-redis-test
```

### Q4: 测试环境的数据存储在哪里？

**A**: 
- **数据卷**：Docker 管理的卷（`lahm_test_mongodb_data`, `lahm_test_redis_data`）
- **日志文件**：`logs-test/` 目录
- **配置文件**：`config-test/` 目录
- **数据文件**：`data-test/` 目录
...
# 删除测试数据卷
docker volume rm lahm_test_mongodb_data
docker volume rm lahm_test_redis_data
```
