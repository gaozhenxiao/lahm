...
本文档整理了 `D:\code\柳暗花明\lahm` 目录下所有股票数据获取相关的函数和方法，按照架构层次和数据类型进行分类。
...
#### 数据源管理器 (`lahm/dataflows/data_source_manager.py`)
```python
...
#### 中国股票数据提供器 (`lahm/dataflows/optimized_china_data.py`)
```python
...
#### 美股数据提供器 (`lahm/dataflows/optimized_us_data.py`)
```python
...
#### 港股数据工具 (`lahm/dataflows/hk_stock_utils.py`)
```python
...
#### Tushare适配器 (`lahm/dataflows/tushare_utils.py`)
```python
...
#### AKShare适配器 (`lahm/dataflows/akshare_utils.py`)
```python
...
#### TDX适配器 (`lahm/dataflows/tdx_utils.py`)
```python
...
#### 股票数据服务 (`lahm/dataflows/stock_data_service.py`)
```python
...
#### 实时新闻工具 (`lahm/dataflows/realtime_news_utils.py`)
```python
...
```python
# 中国A股 - 推荐
from lahm.dataflows import get_china_stock_data_unified, get_china_stock_info_unified

# 美股 - 推荐  
from lahm.dataflows.optimized_us_data import get_us_stock_data_cached
