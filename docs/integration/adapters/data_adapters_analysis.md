...
**关键原则**：
- ✅ `lahm/` 是**独立的核心库**，不依赖 `app/`
- ✅ `app/` 是**Web应用层**，引用和适配 `lahm/`
- ✅ `app/` 中的适配器是为了将 `lahm/` 的功能暴露为Web API

## 问题：为什么有两套数据接口？

当前项目中存在两套数据适配器实现：

1. **`lahm/dataflows/providers/`** - **核心数据提供器**（底层）
2. **`app/services/data_source_adapters.py`** - **Web适配器**（单文件）
3. **`app/services/data_sources/`** - **Web适配器**（模块化）

## 三层架构详解

### 第一层：`lahm/dataflows/providers/` (核心数据提供器)

**位置**: `lahm/dataflows/providers/china/`
...
**角色**: **Web应用层的适配器**，将 `lahm` 的 Provider 包装成适合Web API使用的接口

**特点**:
- 所有适配器在一个文件中（754行）
- 包含：`DataSourceAdapter`（基类）、`TushareAdapter`、`AKShareAdapter`、`BaoStockAdapter`、`DataSourceManager`
- **包装** `lahm.dataflows.providers` 中的 Provider
- 提供**同步接口**，适合Web请求处理
...
| 层次 | 位置 | 角色 | 接口类型 | 返回格式 |
|------|------|------|----------|----------|
| **核心层** | `lahm/dataflows/providers/` | 数据提供器 | 异步+同步 | List[Dict] / DataFrame |
...
3. **最近的修复示例**
   ...
   - 但核心层 `lahm/dataflows/providers/` 只需修改一次
...
**实施步骤**:
1. **保持** `lahm/dataflows/providers/` 不变（核心层）
...
app/services/data_sources/        ← 统一的Web适配层
         ↓ 包装
lahm/dataflows/providers/ ← 核心数据提供器
```
...
### 立即行动
1. **保持** `lahm/dataflows/providers/` 的独立性（不需要修改）
