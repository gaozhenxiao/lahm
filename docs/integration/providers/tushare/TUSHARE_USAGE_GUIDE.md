...
```python
# 分析平安银行
from lahm.dataflows import get_china_stock_fundamentals_tushare
...
```python
# 搜索银行股
from lahm.dataflows import search_china_stocks_tushare
...
```python
from lahm.config.database_manager import get_mongodb_client
...
```python
# 查看当前数据源
from lahm.dataflows import get_current_china_data_source

current = get_current_china_data_source()
print(current)

# 切换数据源（如果需要）
from lahm.dataflows import switch_china_data_source
