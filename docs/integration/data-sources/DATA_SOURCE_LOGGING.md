...
1. **`lahm/dataflows/data_source_manager.py`**
   - 在 `get_stock_data()` 方法中添加数据来源标记
   - 在 `get_stock_info()` 方法中添加数据来源标记
   - 在降级处理中添加数据来源标记

2. **`lahm/dataflows/optimized_china_data.py`**
   - 在 `get_stock_data()` 方法中添加数据来源标记
   - 在 `get_fundamentals_data()` 方法中添加数据来源标记
   - 在缓存、API、备用数据等各个环节添加标记

3. **`lahm/dataflows/optimized_us_data.py`**
   - 在 `get_stock_data()` 方法中添加数据来源标记
   - 区分 FINNHUB、Yahoo Finance、AKShare 等不同来源
   - 在缓存和 API 调用中添加标记

4. **`lahm/dataflows/enhanced_data_adapter.py`**
...
```bash
# 查看所有数据来源日志
tail -f logs/lahm.log | grep "数据来源"

# 查看特定股票的数据来源
tail -f logs/lahm.log | grep "数据来源" | grep "000001"

# 查看 API 调用日志
tail -f logs/lahm.log | grep "数据来源.*API"

# 查看缓存命中日志
tail -f logs/lahm.log | grep "数据来源.*缓存"
```
...
```bash
# 统计各数据来源的使用次数
grep "数据来源" logs/lahm.log | grep -oP '\[数据来源: \K[^\]]+' | sort | uniq -c | sort -rn
```
...
```bash
# 查看 API 失败日志
grep "数据来源.*失败" logs/lahm.log

# 统计失败次数
grep "数据来源.*失败" logs/lahm.log | wc -l
```
