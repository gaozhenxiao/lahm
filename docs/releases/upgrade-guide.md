...
本指南提供柳暗花明各版本之间的升级方法，确保用户能够安全、顺利地升级到最新版本。
...
- 配置管理支持新增厂家和新增模型自动置顶
- 页面切换时的股票详情和报告详情刷新问题已修复
- 单股同步结果支持显示主链路、回退链路和落库状态
- AKShare 单股实时行情新增备份接口与历史日线兜底

### 📋 升级步骤

#### 1. 从 v1.0.0-preview 升级

```bash
# 1. 备份配置文件
cp .env .env.backup.v1.0.0-preview

# 2. 备份数据库
mongodump --out backup_v1.0.1_$(date +%Y%m%d)
...
- ✅ 兼容现有配置结构
- ✅ 无需手工迁移模型配置
- ✅ 历史旧数据缺失时间字段时仍可按追加顺序倒序返回
- ✅ 原有数据源配置和模型配置可继续使用

#### 4. 建议操作

- 升级完成后至少执行一次单股同步验证
- 若使用 Tushare，请避免一分钟内重复触发实时同步
- 若 AKShare 主接口失败，请观察是否已自动切换到备份接口
...
##### 智能新闻过滤
```python
from lahm.utils.news_filter import NewsFilter
...
##### 统一新闻工具
```python
from lahm.tools.unified_news_tool import UnifiedNewsTool
...
# 克隆最新版本
git clone https://github.com/hsliuping/lahm.git .
...
# 本地环境
# 确保使用localhost
MONGODB_URL=mongodb://localhost:27017/lahm
...
- 🐛 [GitHub Issues](https://github.com/hsliuping/lahm/issues)
- 💬 [GitHub Discussions](https://github.com/hsliuping/lahm/discussions)
- 📚 [完整文档](https://github.com/hsliuping/lahm/tree/main/docs)
