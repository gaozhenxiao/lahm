# 2025-10-24 运维指南：从 Docker Hub 更新 柳暗花明‑CN 镜像（含清理数据卷）
...
- Backend（`hsliup/lahm-backend:latest`）
- Frontend（`hsliup/lahm-frontend:latest`）
...
MongoDB 备份（默认 root 账户：`admin` / `lahm123`；认证库 `admin`）：

```bash
# 导出到容器内 /dump
docker exec lahm-mongodb sh -c \
  'mongodump -u admin -p "lahm123" --authenticationDatabase admin -o /dump'
# 拷贝到宿主机（按需修改目标路径）
docker cp lahm-mongodb:/dump ./backup/mongo-$(date +%F)
```
...
docker cp lahm-redis:/data ./backup/redis-$(date +%F)
```
...
```bash
# 查阅含 lahm 的卷名
docker volume ls | grep lahm  # Windows 用 findstr
# 删除两个命名卷
docker volume rm lahm_mongodb_data lahm_redis_data
```
...
- 健康检查：
  - Backend：`/api/health`
  - Nginx：`/health`
- 命名卷：`lahm_mongodb_data`、`lahm_redis_data`
