...
- `lahm/dataflows/providers/china/tushare.py` - Token优先级
- `lahm/dataflows/cache/mongodb_cache_adapter.py` - 改进日志
- `lahm/agents/analysts/news_analyst.py` - 改进日志
...
```bash
# 1. 清空market_quotes集合
mongo lahm --eval "db.market_quotes.deleteMany({})"

# 2. 重启后端服务
# 观察启动日志

# 3. 检查market_quotes是否有数据
mongo lahm --eval "db.market_quotes.countDocuments({})"
...
mongo lahm --eval "db.market_quotes.find({}, {code: 1}).limit(10)"
...
# 1. 备份数据
mongodump --db lahm --out /backup/$(date +%Y%m%d)

# 2. 清空基础数据集合
mongo lahm --eval "db.stock_basic_info.deleteMany({})"
