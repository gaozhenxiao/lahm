...
在 `lahm/dataflows/data_source_manager.py` 中添加：
...
在 `lahm/dataflows/providers/hk/hk_stock.py` 中添加相同的计算逻辑。
...
在 `lahm/dataflows/optimized_china_data.py` 中添加异常处理：
...
在 `lahm/agents/utils/agent_states.py` 中：
...
在 `lahm/dataflows/interface.py` 中：
...
.\vendors\mongodb\mongodb-win32-x86_64-windows-8.0.13\bin\mongodump.exe `
    --host localhost --port 27017 --db lahm `
...
- 下载最新的绿色版压缩包（例如：`Lahm-portable-v0.1.14.zip`）
- 解压到临时目录（例如：`C:\Temp\Lahm-portable-new`）

5. **覆盖程序文件**

```powershell
# 设置路径（根据实际情况修改）
$source = "C:\Temp\Lahm-portable-new"
$target = "当前绿色版目录"  # 例如：C:\Lahm-portable

# 覆盖核心代码
Copy-Item -Path "$source\lahm" -Destination "$target\lahm" -Recurse -Force
...
docker exec lahm-mongodb mongodump \
    ...
    --username admin --password lahm123 --authenticationDatabase admin \
    --db lahm \
    --out /data/db/backup_$(date +%Y%m%d_%H%M%S)

# 或复制备份到宿主机
docker cp lahm-mongodb:/data/db/backup_$(date +%Y%m%d_%H%M%S) ./mongodb_backup_$(date +%Y%m%d_%H%M%S)
```
...
```bash
# Docker 版
docker exec lahm-mongodb mongosh \
    --username admin --password lahm123 --authenticationDatabase admin \
    lahm --eval "
print('股票日线数据:', db.stock_daily_quotes.countDocuments());
print('股票基本信息:', db.stock_basic_info.countDocuments());
...
```bash
# Docker 版
docker exec -i lahm-mongodb mongorestore \
    --username admin --password lahm123 --authenticationDatabase admin \
    --db lahm --drop \
    /data/db/backup_20251106_120000/lahm
```
...
.\vendors\mongodb\mongodb-win32-x86_64-windows-8.0.13\bin\mongosh.exe lahm --eval "
db.stock_daily_quotes.deleteMany({data_source: 'tushare'})
"
```
...
```bash
# Docker 版
docker exec lahm-mongodb mongosh lahm --eval "
db.stock_daily_quotes.deleteMany({data_source: 'tushare'})
"
```
...
.\vendors\mongodb\mongodb-win32-x86_64-windows-8.0.13\bin\mongorestore.exe `
    --host localhost --port 27017 --db lahm --drop `
    "data\backups\mongodb_dump_20251106\lahm"
```

```bash
# Docker 版
docker exec -i lahm-mongodb mongorestore `
    --db lahm --drop /data/db/backup_20251106/lahm
```
