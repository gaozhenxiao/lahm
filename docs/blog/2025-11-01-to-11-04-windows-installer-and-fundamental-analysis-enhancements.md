...
; 基本信息
Name "柳暗花明"
OutFile "柳暗花明-Setup.exe"
InstallDir "$PROGRAMFILES64\柳暗花明"
...
Section "主程序" SecMain
    ...
    ; 创建快捷方式
    CreateDirectory "$SMPROGRAMS\柳暗花明"
    CreateShortcut "$SMPROGRAMS\柳暗花明\柳暗花明.lnk" "$INSTDIR\start.bat"
    CreateShortcut "$DESKTOP\柳暗花明.lnk" "$INSTDIR\start.bat"
    
    ; 写入卸载信息
    WriteUninstaller "$INSTDIR\Uninstall.exe"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\柳暗花明" \
                     "DisplayName" "柳暗花明"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\柳暗花明" \
...
Section "Uninstall"
    ...
    ; 删除快捷方式
    Delete "$DESKTOP\柳暗花明.lnk"
    RMDir /r "$SMPROGRAMS\柳暗花明"
    
    ; 删除注册表
    DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\柳暗花明"
SectionEnd
```
...
```python
# lahm/agents/analysts/fundamentals_analyst.py
...
**核心代码（8个文件）**：
- `lahm/dataflows/optimized_china_data.py` - 添加总市值数据
- `lahm/agents/analysts/fundamentals_analyst.py` - 添加日志
- `lahm/llm/llm_adapter.py` - API Key 热更新
