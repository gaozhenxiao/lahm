...
## 8. 集成与目录建议（现有项目）
- 后端：`app/routers/paper.py`（已有）、扩展 `sse.py`（已有）与新增 `data.py`；`app/services/paper/` 实现 `BrokerSim/OMS/Portfolio`；`lahm/backtest/` 放统一接口与适配器。
- 数据与特征：`dataflows/features/` 与 `dataflows/labels/`；版本冻结与缓存策略。
- 测试：`tests/lahm/paper/` 单元与集成测试；回放与再现性用例。
