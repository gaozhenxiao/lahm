...
```
ModuleNotFoundError: No module named 'lahm.dataflows.news.akshare_utils'
```
...
根据项目架构规范：
- ✅ **所有数据接口必须统一在 `lahm/dataflows/providers/` 目录管理**
...
**根本原因**：
- `lahm/dataflows/news/` 目录下没有 `akshare_utils.py` 文件
- 代码尝试导入不存在的模块
...
**影响范围**：
- `lahm/dataflows/news/realtime_news.py` 中有 3 处错误导入
- `lahm/agents/utils/agent_utils.py` 中有 1 处错误导入
...
**影响范围**：
- `lahm/tools/unified_news_tool.py` 第 135 行

## ✅ 解决方案

### 修复 1：在 AKShareProvider 中添加同步方法

**文件**：`lahm/dataflows/providers/china/akshare.py`
...
**文件**：`lahm/dataflows/news/realtime_news.py`
...
try:
    ...
    from lahm.dataflows.providers.china.akshare import AKShareProvider
...
**文件**：`lahm/agents/utils/agent_utils.py`
...
```python
# 导入AKShare新闻获取函数
from lahm.dataflows.akshare_utils import get_stock_news_em
...
**文件**：`lahm/tools/unified_news_tool.py`
...
```python
try:
    from lahm.dataflows.cache.app_adapter import get_mongodb_client
...
```
❌ [新闻分析] 东方财富新闻获取失败: No module named 'lahm.dataflows.news.akshare_utils'
...
1. ✅ `lahm/dataflows/providers/china/akshare.py`
   - 新增了 `get_stock_news_sync()` 同步方法
   - 提供统一的数据访问接口

2. ✅ `lahm/dataflows/news/realtime_news.py`
   - 修复了 3 处 AKShare 导入错误
   - 改用 `AKShareProvider` 访问数据

3. ✅ `lahm/agents/utils/agent_utils.py`
   - 修复了 1 处 AKShare 导入错误
   - 改用 `AKShareProvider` 访问数据
   - 修正了字段名称映射

4. ✅ `lahm/tools/unified_news_tool.py`
   - 添加了 `max_news` 参数类型转换
...
```python
from lahm.tools.unified_news_tool import UnifiedNewsAnalyzer
...
**修复文件**：
- ✅ `lahm/dataflows/providers/china/akshare.py` - 新增同步方法
- ✅ `lahm/dataflows/news/realtime_news.py` - 3 处修复
- ✅ `lahm/agents/utils/agent_utils.py` - 1 处修复
- ✅ `lahm/tools/unified_news_tool.py` - 类型转换
