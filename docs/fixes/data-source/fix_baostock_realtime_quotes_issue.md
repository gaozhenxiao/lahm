...
2. **代码实现**
   ...
   在 `lahm/dataflows/providers/china/baostock.py` 中：
...
```bash
# Docker 环境
docker logs lahm-backend | grep "BaoStock"
...
```bash
docker logs -f lahm-backend | grep "BaoStock日K线"
...
1. **lahm/dataflows/providers/china/baostock.py** - BaoStock Provider 实现
