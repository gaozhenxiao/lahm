...
TDX（通达信）数据源已从 柳暗花明 项目中完全移除，原因如下：
...
### 5. 配置文件
```python
# lahm/config/providers_config.py
...
#### 4. 测试新数据源
```python
from lahm.dataflows import get_china_stock_data_unified
...
### 受影响的文件
1. ✅ `lahm/dataflows/data_source_manager.py` - 移除 TDX 枚举和相关方法
2. ✅ `lahm/config/providers_config.py` - 移除 TDX 配置
3. ⚠️ `lahm/dataflows/providers/china/tdx.py` - 保留但标记为已弃用
...
### 1. TDX 文件保留
`lahm/dataflows/providers/china/tdx.py` 文件暂时保留，但已标记为已弃用：
- 不会被主动调用
- 仅用于向后兼容
- 将在未来版本中完全删除
...
```python
# ❌ 不再支持
from lahm.dataflows.providers.china.tdx import get_tdx_provider
...
### 保留的内容
- ✅ `lahm/dataflows/providers/china/tdx.py` 文件（标记为已弃用）
