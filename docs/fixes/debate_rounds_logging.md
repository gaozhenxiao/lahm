...
**文件**: `lahm/graph/trading_graph.py` (第262行)
...
**文件**: `lahm/graph/conditional_logic.py`
...
**多头研究员** (`lahm/agents/researchers/bull_researcher.py`):
```python
logger.info(f"🐂 [多头研究员] 发言完成，计数: {old_count} -> {new_count}")
```

**空头研究员** (`lahm/agents/researchers/bear_researcher.py`):
```python
...
**激进风险分析师** (`lahm/agents/risk_mgmt/aggresive_debator.py`):
```python
logger.info(f"🔥 [激进风险分析师] 发言完成，计数: {old_count} -> {new_count}")
```

**保守风险分析师** (`lahm/agents/risk_mgmt/conservative_debator.py`):
```python
logger.info(f"🛡️ [保守风险分析师] 发言完成，计数: {old_count} -> {new_count}")
```

**中性风险分析师** (`lahm/agents/risk_mgmt/neutral_debator.py`):
```python
...
搜索日志中的配置信息：
```powershell
Get-Content logs/lahm.log | Select-String "ConditionalLogic.*初始化|辩论轮次|风险讨论轮次"
```
...
搜索投资辩论相关日志：
```powershell
Get-Content logs/lahm.log | Select-String "投资辩论控制|多头研究员|空头研究员"
```
...
搜索风险讨论相关日志：
```powershell
Get-Content logs/lahm.log | Select-String "风险讨论控制|激进风险|保守风险|中性风险"
```
