...
```bash
# 本地构建
docker build -t hsliup/lahm-backend:latest -f Dockerfile.backend .

# 推送到 Docker Hub
docker push hsliup/lahm-backend:latest
```
...
# 验证 pandoc 是否安装
docker exec -it lahm-backend pandoc --version
```
...
```bash
# 验证 wkhtmltopdf 是否安装
docker exec -it lahm-backend wkhtmltopdf --version
...
```bash
# 检查中文字体是否安装
docker exec -it lahm-backend fc-list :lang=zh
...
如有问题，请查看：
- 后端日志：`docker logs lahm-backend`
- 应用日志：`docker exec lahm-backend cat /app/logs/lahm.log`
