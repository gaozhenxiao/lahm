...
**改动**:
- ✅ 复制文件到 `lahm/dataflows/news/chinese_finance.py`
...
**改动**:
- ✅ 复制文件到 `lahm/dataflows/providers/china/fundamentals_snapshot.py`
...
- 导入路径变更：
  ```python
  # 旧
  from lahm.dataflows.fundamentals_snapshot import get_cn_fund_snapshot
  
  # 新
  from lahm.dataflows.providers.china.fundamentals_snapshot import get_cn_fund_snapshot
  ```
...
- **原因**: 被广泛使用（26 处引用）
- **使用位置**:
  - `lahm/models/stock_data_models.py` - 2 处
...
- **使用位置**:
  - `lahm/agents/utils/agent_utils.py` - 4 处
  - `lahm/agents/analysts/market_analyst.py` - 2 处
...
```bash
.\.venv\Scripts\python -c "from lahm.dataflows.news.chinese_finance import ChineseFinanceDataAggregator; from lahm.dataflows.providers.china.fundamentals_snapshot import get_cn_fund_snapshot; print('✅ 导入测试成功')"
```
