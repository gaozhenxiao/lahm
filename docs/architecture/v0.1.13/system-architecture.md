...
柳暗花明 是一个基于多智能体协作的金融交易决策框架，采用 LangGraph 构建智能体工作流，支持中国A股、港股和美股的全面分析。系统通过模块化设计实现高度可扩展性和可维护性。
...
#### LLM适配器架构
**文件位置**: `lahm/llm_adapters/`

```python
from langchain_openai import ChatOpenAI
from langchain_anthropic import ChatAnthropic
from langchain_google_genai import ChatGoogleGenerativeAI
from lahm.llm_adapters import ChatDashScope, ChatDashScopeOpenAI, ChatGoogleOpenAI
...
#### LahmGraph 主控制器
**文件位置**: `lahm/graph/trading_graph.py`
...
**文件位置**: `lahm/graph/conditional_logic.py`
...
#### 状态管理系统
**文件位置**: `lahm/agents/utils/agent_states.py`
...
```python
# 分析师创建函数
from lahm.agents.analysts import (
...
# 研究员创建函数
from lahm.agents.researchers import (
    create_bull_researcher,
    create_bear_researcher
)

# 交易员创建函数
from lahm.agents.trader import create_trader

# 风险管理创建函数
from lahm.agents.risk_mgmt import (
...
# 管理层创建函数
from lahm.agents.managers import (
...
#### Toolkit 统一工具包
**文件位置**: `lahm/agents/utils/agent_utils.py`
...
#### 数据流接口
**文件位置**: `lahm/dataflows/interface.py`
...
#### 记忆管理系统
**文件位置**: `lahm/agents/utils/memory.py`
...
DEFAULT_CONFIG = {
    ...
    "results_dir": os.getenv("LAHM_RESULTS_DIR", "./results"),
    "data_dir": os.path.join(os.path.expanduser("~"), "Documents", "柳暗花明", "data"),
...
sequenceDiagram
    ...
    participant Graph as LahmGraph
...
ta = LahmGraph(config=custom_config)
```

## 📊 监控和观测

### 日志系统
**文件位置**: `lahm/utils/logging_init.py`
...
```python
# 智能体执行时间监控
from lahm.utils.tool_logging import log_analyst_module
...
```python
# 1. 创建智能体文件
# lahm/agents/analysts/custom_analyst.py
...
```python
# 1. 创建数据源适配器
# lahm/dataflows/custom_data_source.py
...
```python
# 1. 创建LLM适配器
# lahm/llm_adapters/custom_llm.py
...
柳暗花明 系统架构通过模块化设计、智能体协作和多源数据融合，为复杂的金融决策提供了强大、可扩展和高性能的技术基础。系统支持多种LLM提供商、数据源和部署方式，能够适应不同的使用场景和性能要求。