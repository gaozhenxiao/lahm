...
柳暗花明 现在支持多架构 Docker 镜像构建，可以在 AMD64 (x86_64) 和 ARM64 (ARM) 架构上运行。
...
docker buildx build --platform linux/arm64 \
  ...
  -t lahm-backend:arm64 \
  --load .

# 构建 ARM64 前端
docker buildx build --platform linux/arm64 \
  -f Dockerfile.frontend \
  -t lahm-frontend:arm64 \
  --load .
```
...
docker buildx build --platform linux/amd64,linux/arm64 \
  -f Dockerfile.backend \
  -t your-registry/lahm-backend:latest \
  --push .

# 构建并推送前端
docker buildx build --platform linux/amd64,linux/arm64 \
  -f Dockerfile.frontend \
  -t your-registry/lahm-frontend:latest \
  --push .
```
...
# 查看远程镜像支持的架构
docker buildx imagetools inspect your-registry/lahm-backend:latest
```
...
# 构建 ARM64 镜像
docker buildx build --platform linux/arm64 -f Dockerfile.backend -t lahm-backend:arm64 --load .
```
...
1. **使用预构建镜像**（推荐）：
   ```bash
   docker pull your-registry/lahm-backend:latest
   ```
...
3. **使用 Docker 构建缓存**：
   ```bash
   docker buildx build --cache-from=type=registry,ref=your-registry/lahm-backend:buildcache \
                       --cache-to=type=registry,ref=your-registry/lahm-backend:buildcache \
                       ...
   ```
