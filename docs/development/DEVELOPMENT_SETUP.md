...
本文档介绍如何配置柳暗花明的开发环境，包括Docker映射配置和快速调试方法。
...
# 查看容器日志
docker logs 柳暗花明-web --follow

# 进入容器调试
docker exec -it 柳暗花明-web bash
```
...
# 检查依赖
docker exec 柳暗花明-web pip list | grep pandoc
```

### 3. 文件同步验证
```bash
# 检查文件是否同步
docker exec 柳暗花明-web ls -la /app/web/utils/

# 检查文件内容
docker exec 柳暗花明-web head -10 /app/test_conversion.py
```
...
2. **运行转换测试** - `docker exec 柳暗花明-web python test_conversion.py`
3. **检查结果** - 查看生成的测试文件
...
2. **相关文件**
   ```
   lahm/agents/researchers/bull_researcher.py
   lahm/agents/researchers/bear_researcher.py
   lahm/agents/managers/research_manager.py
   lahm/agents/managers/risk_manager.py
   ```
