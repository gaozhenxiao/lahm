...
## 1. 背景与目标
目前项目中技术指标散落在多个位置：
- lahm/dataflows/tdx_utils.py：内联实现了 MACD、布林带等少量指标（使用 pandas 计算，字段名大小写混杂，如 `Close`）。
- lahm/dataflows/stockstats_utils.py：借助第三方 `stockstats` 包通过访问列名形式触发计算（如 `df['macd']`）。
...
### 3.1 函数签名
```python
# lahm/tools/analysis/indicators.py
...
## 5. 兼容与迁移计划
- 新增模块：`lahm/tools/analysis/indicators.py`
- 迁移/封装：
