...
```bash
# 从 Docker Hub 拉取（最快）
docker pull hsliuping/lahm-backend:v1.0.0-preview-amd64
docker pull hsliuping/lahm-frontend:v1.0.0-preview-amd64
```
...
# 3. 在其他机器上拉取
docker pull your-dockerhub-username/lahm-backend:v1.0.0-preview-amd64
```
...
# image: hsliuping/lahm-backend:v1.0.0-preview-amd64
...
3. **重新创建 builder**
   ```bash
   docker buildx rm lahm-builder-amd64
   ./scripts/build-amd64.sh
   ```
...
```bash
# 查看镜像详细信息
docker inspect lahm-backend:v1.0.0-preview | grep Architecture

# 或使用 buildx
docker buildx imagetools inspect lahm-backend:v1.0.0-preview
```
...
1. 查看构建日志
2. 检查 Docker 版本和配置
3. 提交 Issue：[GitHub Issues](https://github.com/hsliuping/lahm/issues)
