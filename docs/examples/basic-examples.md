...
本文档提供了 柳暗花明 框架的基本使用示例，帮助您快速上手并了解各种功能的使用方法。
...
def batch_analysis(symbols, date):
    ...
    ta = LahmGraph(debug=False, config=config)
...
# 使用示例
ta = LahmGraph(debug=False, config=DEFAULT_CONFIG.copy())
state, decision = ta.propagate("AAPL", "2024-01-15")
...
这些基本示例展示了 柳暗花明 框架的主要功能和使用模式。您可以根据自己的需求修改和扩展这些示例。
