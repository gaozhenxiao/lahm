...
```bash
# 查看 MongoDB 容器日志
docker logs lahm-mongodb | grep "mongo-init.js"
```
...
mongodb://admin:lahm123@mongodb:27017/lahm?authSource=admin

# 使用应用用户（lahm 数据库）
mongodb://lahm:lahm123@mongodb:27017/lahm?authSource=admin
```
...
```bash
# 1. 检查容器是否在同一网络
docker network inspect lahm-network

# 2. 从应用容器 ping MongoDB 容器
docker exec -it lahm-backend ping mongodb

# 3. 测试端口连接
docker exec -it lahm-backend nc -zv mongodb 27017
```

**解决方案**：
- 确保所有容器都在 `lahm-network` 网络中
...
```bash
# 进入应用容器
docker exec -it lahm-backend bash
...
# 查看容器日志
docker logs lahm-mongodb --tail 100

# 检查健康状态
docker inspect lahm-mongodb | grep -A 10 Health
```
...
# 方法 2：使用 Python
docker exec -it lahm-backend python3 -c "
from pymongo import MongoClient
client = MongoClient('mongodb://admin:lahm123@mongodb:27017/lahm?authSource=admin')
...
```bash
# 查看应用容器的环境变量
docker exec -it lahm-backend env | grep MONGODB
```
...
environment:
  ...
  MONGODB_PASSWORD: "lahm123"
  MONGODB_DATABASE: "lahm"
  MONGODB_AUTH_SOURCE: "admin"
  MONGODB_CONNECTION_STRING: "mongodb://admin:lahm123@mongodb:27017/lahm?authSource=admin"
```
...
environment:
  MONGODB_HOST: "mongodb"
  MONGODB_PORT: "27017"
  MONGODB_USERNAME: "lahm"
  MONGODB_PASSWORD: "lahm123"
  MONGODB_DATABASE: "lahm"
  MONGODB_AUTH_SOURCE: "admin"
  MONGODB_CONNECTION_STRING: "mongodb://lahm:lahm123@mongodb:27017/lahm?authSource=admin"
```
...
db.createUser({
  ...
  pwd: 'lahm123',
  roles: [
    { role: 'readWrite', db: 'lahm' }
  ]
})

# 3. 验证用户
db.auth('lahm', 'lahm123')
