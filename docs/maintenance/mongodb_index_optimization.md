...
```bash
# Docker 环境
docker exec -it lahm-mongodb mongosh -u admin -p your_password --authenticationDatabase admin

# 本地环境
mongosh mongodb://localhost:27017/lahm
```
...
```bash
# 查看 MongoDB 慢查询日志
docker logs lahm-mongodb | grep "Slow query"
```
...
0 2 1 * * cd /path/to/Lahm && python scripts/maintenance/optimize_mongodb_indexes.py
```
