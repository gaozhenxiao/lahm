...
**`lahm/config/providers_config.py` (第 23-32 行)**:
```python
...
**`lahm/dataflows/providers/china/tushare.py` (第 31-48 行)**:
```python
def __init__(self):
...
**核心问题**: `lahm/config/providers_config.py` 中的 `DataSourceConfig` 类在初始化时（第 18-19 行）直接从环境变量读取 Token：
...
- **位置**: `lahm/config/providers_config.py:131-139`
- **问题**: 配置在应用启动时固定，运行时修改数据库配置不会生效

**Bug 3**: Tushare Provider 不从数据库读取配置
- **位置**: `lahm/dataflows/providers/china/tushare.py:34`
...
**修改 `lahm/config/providers_config.py`**，添加重新加载方法：
...
**修改 `lahm/dataflows/providers/china/tushare.py`**:
...
2. `lahm/config/providers_config.py` - 数据源配置管理
3. `lahm/dataflows/providers/china/tushare.py` - Tushare Provider
