...
本文档详细介绍柳暗花明在Docker环境中的配置方法，包括环境变量设置、服务配置、网络配置和数据持久化配置。
...
```yaml
  mongo-express:
    image: mongo-express
    container_name: lahm-mongo-express
    ports:
  ...
  redis-commander:
    ...
    container_name: lahm-redis-commander
    ports:
...
1. **服务连接失败**
   ...
   docker exec 柳暗花明-web ping redis
   ```
...
3. **环境变量问题**
   ```bash
   # 检查环境变量
   docker exec 柳暗花明-web env | grep MONGODB
   ```
