...
柳暗花明 现在提供了更精细的在线工具控制机制，您可以通过环境变量灵活配置系统的在线/离线行为，而不再依赖于特定LLM提供商的启用状态。
...
```bash
# 检查当前配置
python -c "from lahm.default_config import DEFAULT_CONFIG; print(DEFAULT_CONFIG)"
...
通过这个新的配置系统，您可以更精确地控制柳暗花明的在线行为，在功能需求和成本控制之间找到最佳平衡点。