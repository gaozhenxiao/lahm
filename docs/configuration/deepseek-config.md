...
DeepSeek V3是一个性能强大、性价比极高的大语言模型，在推理、代码生成和中文理解方面表现优秀。本指南将详细介绍如何在柳暗花明中配置和使用DeepSeek V3。
...
- ❌ **deepseek-reasoner**：不支持工具调用，不适用于柳暗花明的智能体架构
...
```python
from lahm.llm.deepseek_adapter import create_deepseek_adapter
...
1. **检查配置**：
```python
from lahm.llm.deepseek_adapter import DeepSeekAdapter
