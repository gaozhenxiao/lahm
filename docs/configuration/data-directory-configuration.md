...
本指南详细说明如何在柳暗花明中配置数据目录路径，解决路径相关问题，并提供多种配置方式。

This guide explains how to configure data directory paths in 柳暗花明, resolve path-related issues, and provides multiple configuration methods.

## 概述 | Overview

柳暗花明支持灵活的数据目录配置，允许用户：
- 自定义数据存储位置
- 通过环境变量配置
- 使用CLI命令管理
- 自动创建必要的目录结构

柳暗花明 supports flexible data directory configuration, allowing users to:
...
```cmd
# 设置数据目录
set LAHM_DATA_DIR=C:\MyTradingData

# 设置缓存目录
set LAHM_CACHE_DIR=C:\MyTradingData\cache

# 设置结果目录
set LAHM_RESULTS_DIR=C:\MyTradingData\results
```

#### Linux/macOS
```bash
# 设置数据目录
export LAHM_DATA_DIR="/home/user/trading-data"

# 设置缓存目录
export LAHM_CACHE_DIR="/home/user/trading-data/cache"

# 设置结果目录
export LAHM_RESULTS_DIR="/home/user/trading-data/results"
```

#### .env文件配置 | .env File Configuration
```env
# 在项目根目录创建.env文件
LAHM_DATA_DIR=/path/to/your/data
LAHM_CACHE_DIR=/path/to/your/cache
LAHM_RESULTS_DIR=/path/to/your/results
```

### 3. 程序化配置 | Programmatic Configuration

```python
from lahm.dataflows.config import set_data_dir, get_data_dir
...
- **Windows**: `C:\Users\{username}\Documents\柳暗花明\data`
- **Linux/macOS**: `~/Documents/柳暗花明/data`
...
- `lahm/config/config_manager.py` - 配置管理器
- `lahm/dataflows/config.py` - 数据流配置
- `cli/main.py` - CLI命令实现
