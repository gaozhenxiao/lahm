...
配置桥接模块 (`app/core/config_bridge.py`) 负责将统一配置系统中的配置桥接到环境变量，让 柳暗花明 核心库能够使用用户在 Web 界面中配置的参数。
...
柳暗花明 的数据源配置管理器 (`lahm/config/providers_config.py`) 从环境变量读取：
...
柳暗花明 的运行时设置 (`lahm/config/runtime_settings.py`) 从环境变量读取：
