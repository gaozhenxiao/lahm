...
**配置桥接** 是一个将 **数据库中的配置** 同步到 **环境变量** 和 **文件系统** 的机制，目的是让 柳暗花明 核心库能够使用统一配置系统中的配置。
...
2025-10-16 19:24:35 | app.config_bridge | INFO | ✓ 桥接 MONGODB_DATABASE_NAME: lahm
```
...
- `MONGODB_DATABASE_NAME=lahm`：数据库名称
...
**说明**：
- 将大模型配置桥接到环境变量
- `LAHM_DEFAULT_MODEL=qwen-turbo`：默认模型
- `LAHM_QUICK_MODEL=qwen-turbo`：快速分析模型
- `LAHM_DEEP_MODEL=qwen-plus`：深度分析模型
...
```python
import os
print(os.environ.get('LAHM_DEFAULT_MODEL'))
