...
| 环境变量 | 值 | 来源 |
|---------|---|------|
| `LAHM_DEFAULT_MODEL` | `qwen-turbo` | 统一配置 |
| `LAHM_QUICK_MODEL` | `qwen-turbo` | 统一配置 |
| `LAHM_DEEP_MODEL` | `qwen-max` | 统一配置 |
| 数据源细节配置 | 2 项 | 统一配置 |
...
```
2025-10-07 09:28:20 | app.config_bridge    | WARNING  | ⚠️  柳暗花明 将使用 .env 文件中的配置
```
...
配置桥接功能基本实现并测试成功！虽然还有一些小问题需要修复，但核心功能已经正常工作。用户在配置管理界面中设置的配置现在可以被 柳暗花明 核心库使用了。
