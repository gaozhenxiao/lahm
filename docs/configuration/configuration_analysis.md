...
#### 4.1.2 ConfigManager (柳暗花明)

**文件**: `lahm/config/config_manager.py`  
**类**: `ConfigManager`  
**用途**: 管理 柳暗花明 库的配置（旧格式）
...
**文件**: `lahm/config/runtime_settings.py`
...
**使用示例**:
```python
from lahm.config.runtime_settings import get_float, get_bool
...
**影响范围**:
- `lahm/config/config_manager.py` (旧系统)
...
**建议**:
- 废弃 `lahm/config/config_manager.py`
- 迁移所有配置到 MongoDB
- 保留 JSON 文件作为备份/导出格式
...
| `lahm/config/config_manager.py` | Python | 配置管理器（旧） | 代码 |
...
柳暗花明 系统的配置管理较为复杂，存在多套配置系统并存的情况。主要问题包括：
...
**任务清单**:
- [ ] 标记 `lahm/config/config_manager.py` 为废弃
