...
2. **柳暗花明 核心库**：仍使用旧版配置（`DEFAULT_CONFIG` + 环境变量）
...
| 模块 | 文件 | 问题 |
|------|------|------|
| 柳暗花明 核心 | `lahm/graph/trading_graph.py` | 使用 `DEFAULT_CONFIG` + `os.getenv()` |
...
| 配置管理器 | `lahm/config/config_manager.py` | 独立的配置系统 |

## 🎯 迁移目标

### 目标 1：柳暗花明 使用统一配置

**修改文件**：`lahm/graph/trading_graph.py`
...
- [ ] 修改 `lahm/graph/trading_graph.py`
...
`lahm` 库不应该直接依赖 `app` 模块，需要通过以下方式解决：
...
# 将统一配置导出为 JSON 文件
# 柳暗花明 从文件读取
config_file = Path("~/.lahm/config.json")
```
...
# app 层在启动时将配置写入环境变量
# 柳暗花明 从环境变量读取
os.environ['LAHM_QUICK_MODEL'] = unified_config.get_quick_analysis_model()
os.environ['LAHM_DEEP_MODEL'] = unified_config.get_deep_analysis_model()
```
