...
本文档说明如何测试配置迁移功能，验证用户在配置向导或配置管理中设置的配置是否被 柳暗花明 核心库正确使用。

## 🎯 测试目标

验证以下功能：

1. ✅ 配置向导设置的 API 密钥被 柳暗花明 使用
2. ✅ 配置管理添加的模型被 柳暗花明 使用
3. ✅ 配置更新后可以热重载（无需重启）
4. ✅ 环境变量桥接正常工作
5. ✅ 配置优先级正确（统一配置 > 环境变量）
...
print(f"LAHM_DEFAULT_MODEL: {os.environ.get('LAHM_DEFAULT_MODEL', 'NOT SET')}")
```
...
在 `lahm/graph/trading_graph.py` 中添加日志：
