...
柳暗花明 提供了统一的配置系统，所有配置通过 `.env` 文件管理。本指南详细介绍了所有可用的配置选项和最佳实践，包括v0.1.7新增的Docker部署和报告导出配置。
...
export LAHM_RESULTS_DIR="/custom/results/path"
```
...
LAHM_RESULTS_DIR=./custom_results
LAHM_LOG_LEVEL=INFO
```
...
通过合理的配置，您可以根据不同的使用场景优化 柳暗花明 的性能和成本。
