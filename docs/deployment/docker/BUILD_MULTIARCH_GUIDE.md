...
```bash
# 创建支持多架构的 builder
docker buildx create --name lahm-builder --use --platform linux/amd64,linux/arm64
...
```
NAME/NODE                  DRIVER/ENDPOINT STATUS  PLATFORMS
lahm-builder *    docker-container
  lahm-builder0   unix:///var/run/docker.sock running linux/amd64*, linux/arm64*, ...
```
...
步骤4: 构建并推送后端镜像（多架构）
  镜像名称: your-dockerhub-username/lahm-backend
  目标架构: linux/amd64,linux/arm64
  开始时间: 2025-10-20 10:00:00
  
  构建并推送: your-dockerhub-username/lahm-backend:v1.0.0-preview
  [构建过程输出...]
...
步骤5: 构建并推送前端镜像（多架构）
  镜像名称: your-dockerhub-username/lahm-frontend
  目标架构: linux/amd64,linux/arm64
  开始时间: 2025-10-20 10:20:00
  
  构建并推送: your-dockerhub-username/lahm-frontend:v1.0.0-preview
  [构建过程输出...]
...
步骤6: 验证镜像架构
  验证后端镜像: your-dockerhub-username/lahm-backend:v1.0.0-preview
  Platform:  linux/amd64
  Platform:  linux/arm64
  
  验证前端镜像: your-dockerhub-username/lahm-frontend:v1.0.0-preview
  Platform:  linux/amd64
  Platform:  linux/arm64
...
已发布的镜像（支持 linux/amd64,linux/arm64）：
  后端: your-dockerhub-username/lahm-backend:v1.0.0-preview
  后端: your-dockerhub-username/lahm-backend:latest
  前端: your-dockerhub-username/lahm-frontend:v1.0.0-preview
  前端: your-dockerhub-username/lahm-frontend:latest
...
```bash
# 查看后端镜像支持的架构
docker buildx imagetools inspect your-dockerhub-username/lahm-backend:latest

# 查看前端镜像支持的架构
docker buildx imagetools inspect your-dockerhub-username/lahm-frontend:latest
```

输出示例：
```
Name:      your-dockerhub-username/lahm-backend:latest
MediaType: application/vnd.docker.distribution.manifest.list.v2+json
Digest:    sha256:abc123...

Manifests:
  Name:      your-dockerhub-username/lahm-backend:latest@sha256:def456...
  MediaType: application/vnd.docker.distribution.manifest.v2+json
  Platform:  linux/amd64
  
  Name:      your-dockerhub-username/lahm-backend:latest@sha256:ghi789...
  MediaType: application/vnd.docker.distribution.manifest.v2+json
  Platform:  linux/arm64
```
...
2. 点击 `lahm-backend` 或 `lahm-frontend`
3. 点击 `Tags` 标签页
...
docker pull your-dockerhub-username/lahm-backend:latest
docker pull your-dockerhub-username/lahm-frontend:latest
```
