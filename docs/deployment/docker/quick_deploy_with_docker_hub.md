...
| Tushare | https://tushare.pro/weborder/#/login?reg=tacn | 专业金融数据（可选） |
...
```bash
docker restart lahm-backend

# 等待服务重启（约 10-20 秒）
docker logs -f lahm-backend
```
...
如果确实想完全覆盖数据，可以使用：
```bash
docker exec -it lahm-backend python scripts/import_config_and_create_user.py --overwrite
```
...
```bash
# 导出 MongoDB 数据
docker exec lahm-mongodb mongodump \
  -u admin -p lahm123 --authenticationDatabase admin \
  -d lahm -o /data/backup

# 复制备份到宿主机
docker cp lahm-mongodb:/data/backup ./mongodb_backup
```
