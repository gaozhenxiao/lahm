# 柳暗花明 多架构 Docker 镜像构建指南

> 🏗️ 支持在 ARM 和 x86_64 架构上运行 柳暗花明

## 📋 概述

柳暗花明 支持构建多架构 Docker 镜像，可以在以下平台上运行：
...
docker buildx build \
  ...
  -t your-dockerhub-username/lahm-backend:v1.0.0 \
  --push \
  .
...
docker buildx build \
  ...
  -t your-dockerhub-username/lahm-frontend:v1.0.0 \
  --push \
  .
...
docker pull your-dockerhub-username/lahm-backend:v1.0.0
docker pull your-dockerhub-username/lahm-frontend:v1.0.0
```
...
services:
  backend:
    image: your-dockerhub-username/lahm-backend:v1.0.0
    # 注释掉 build 部分
    # build:
    #   context: .
  ...
  frontend:
    image: your-dockerhub-username/lahm-frontend:v1.0.0
    # 注释掉 build 部分
    # build:
    #   context: .
...
```bash
docker buildx imagetools inspect your-dockerhub-username/lahm-backend:v1.0.0
```

输出示例：

```
Name:      your-dockerhub-username/lahm-backend:v1.0.0
MediaType: application/vnd.docker.distribution.manifest.list.v2+json
Digest:    sha256:abc123...

Manifests:
  Name:      your-dockerhub-username/lahm-backend:v1.0.0@sha256:def456...
  MediaType: application/vnd.docker.distribution.manifest.v2+json
  Platform:  linux/amd64

  Name:      your-dockerhub-username/lahm-backend:v1.0.0@sha256:ghi789...
  MediaType: application/vnd.docker.distribution.manifest.v2+json
...
```bash
docker inspect lahm-backend:v1.0.0 | grep Architecture
```
    ...
    steps:
      ...
      - name: Build and push
        ...
        with:
          ...
          push: true
          tags: ${{ secrets.DOCKERHUB_USERNAME }}/lahm-backend:${{ github.ref_name }}
```
...
docker buildx build \
  --platform linux/amd64,linux/arm64 \
  --cache-from type=registry,ref=your-dockerhub-username/lahm-backend:buildcache \
  --cache-to type=registry,ref=your-dockerhub-username/lahm-backend:buildcache,mode=max \
  -t your-dockerhub-username/lahm-backend:v1.0.0 \
  --push \
  .
```
