...
本指南介绍如何将柳暗花明的Docker镜像发布到Docker Hub。
...
services:
  mongodb:
    image: mongo:4.4
    container_name: lahm-mongodb
    restart: unless-stopped
    ports:
      - "27017:27017"
    volumes:
      - lahm_mongodb_data_v1:/data/db
    environment:
      TZ: "Asia/Shanghai"
    networks:
      - lahm-network
  ...
  redis:
    ...
    container_name: lahm-redis
    restart: unless-stopped
    ports:
      - "6379:6379"
    volumes:
      - lahm_redis_data_v1:/data
    environment:
      TZ: "Asia/Shanghai"
    networks:
      - lahm-network
  ...
  backend:
    image: YOUR_DOCKERHUB_USERNAME/lahm-backend:latest
    container_name: lahm-backend
    restart: unless-stopped
    ports:
      - "8000:8000"
    env_file:
    ...
    environment:
      TZ: "Asia/Shanghai"
      MONGODB_URL: "mongodb://mongodb:27017/lahm"
  ...
  frontend:
    image: YOUR_DOCKERHUB_USERNAME/lahm-frontend:latest
    container_name: lahm-frontend
    restart: unless-stopped
    ports:
      - "3000:80"
    environment:
  ...
  build-and-push:
    ...
    steps:
      ...
      - name: Extract metadata for backend
        ...
        with:
          images: ${{ secrets.DOCKERHUB_USERNAME }}/lahm-backend
          tags: |
            type=ref,event=tag
      ...
      - name: Extract metadata for frontend
        ...
        uses: docker/metadata-action@v5
        with:
          images: ${{ secrets.DOCKERHUB_USERNAME }}/lahm-frontend
          tags: |
            type=ref,event=tag
            type=raw,value=latest
            type=semver,pattern={{version}}
