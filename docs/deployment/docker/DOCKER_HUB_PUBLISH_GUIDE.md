...
本文档说明如何构建 柳暗花明 的 Docker 镜像并发布到 Docker Hub。
...
### 后端镜像
- `your-username/lahm-backend:v1.0.0-preview`
- `your-username/lahm-backend:latest`

### 前端镜像
- `your-username/lahm-frontend:v1.0.0-preview`
...
2. **检查镜像**
   - 确认 `lahm-backend` 和 `lahm-frontend` 都已发布
...
services:
  lahm-backend:
    image: hsliuping/lahm-backend:latest  # 替换为你的用户名
    
  lahm-frontend:
    image: hsliuping/lahm-frontend:latest  # 替换为你的用户名
```
...
```bash
# 例如推送到阿里云容器镜像服务
BACKEND_IMAGE_REMOTE="registry.cn-hangzhou.aliyuncs.com/your-namespace/lahm-backend"
FRONTEND_IMAGE_REMOTE="registry.cn-hangzhou.aliyuncs.com/your-namespace/lahm-frontend"
```
