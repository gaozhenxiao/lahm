# 柳暗花明 v1.0.0-preview Docker部署指南

> 🐳 使用Docker快速部署柳暗花明（前后端分离架构）

## 📋 架构说明

柳暗花明 v1.0.0-preview采用**前后端分离架构**：
...
```bash
# 1. 克隆仓库
git clone https://github.com/hsliuping/柳暗花明.git
cd 柳暗花明
...
```bash
# 备份MongoDB数据
docker exec lahm-mongodb mongodump --out /data/backup

# 恢复MongoDB数据
docker exec lahm-mongodb mongorestore /data/backup

# 清理Redis缓存
docker exec lahm-redis redis-cli -a lahm123 FLUSHALL

# 查看MongoDB数据
docker exec -it lahm-mongodb mongo -u admin -p lahm123
```
...
# 进入前端容器
docker exec -it lahm-frontend sh
...
docker exec lahm-mongodb mongodump --out /data/backup_$DATE
```
