...
柳暗花明采用本地构建Docker镜像的方式，而不是提供预构建镜像。本文档详细说明了Docker镜像的构建过程、优化方法和常见问题解决方案。
...
```bash
# 为镜像打标签
docker build -t lahm:v0.1.7 .
docker build -t lahm:latest .

# 推送到私有仓库 (可选)
docker tag lahm:latest your-registry/lahm:latest
docker push your-registry/lahm:latest
```
