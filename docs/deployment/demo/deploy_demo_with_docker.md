...
**柳暗花明** 是一个基于多智能体架构的 AI 股票分析系统，支持：
...
```bash
# 必须执行此命令
docker exec -it lahm-backend python scripts/import_config_and_create_user.py
```
...
2. 检查 Docker 容器日志：`docker logs lahm-backend`
3. 确认是否遗漏了上述关键步骤
...
**推荐**：如果只是运行 柳暗花明，选择 **Hyper-V 模式**更简单！
...
```powershell
# 1. 创建项目目录
New-Item -ItemType Directory -Path "$env:USERPROFILE\lahm-demo" -Force
Set-Location "$env:USERPROFILE\lahm-demo"

# 2. 下载部署文件
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/hsliuping/柳暗花明/v1.0.0-preview/docker-compose.hub.nginx.yml" -OutFile "docker-compose.hub.nginx.yml"

# 3. 下载环境配置文件
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/hsliuping/柳暗花明/v1.0.0-preview/.env.docker" -OutFile ".env"
...
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/hsliuping/柳暗花明/v1.0.0-preview/nginx/nginx.conf" -OutFile "nginx\nginx.conf"
...
# 2. 下载部署文件
wget https://raw.githubusercontent.com/hsliuping/柳暗花明/v1.0.0-preview/docker-compose.hub.nginx.yml

# 3. 下载环境配置文件
wget https://raw.githubusercontent.com/hsliuping/柳暗花明/v1.0.0-preview/.env.docker -O .env
...
# 5. 下载 Nginx 配置文件
mkdir -p nginx
wget https://raw.githubusercontent.com/hsliuping/柳暗花明/v1.0.0-preview/nginx/nginx.conf -O nginx/nginx.conf
...
# 2. 下载 ARM 架构部署文件（重要！）
curl -O https://raw.githubusercontent.com/hsliuping/柳暗花明/v1.0.0-preview/docker-compose.hub.nginx.arm.yml

# 3. 下载环境配置文件
curl -o .env https://raw.githubusercontent.com/hsliuping/柳暗花明/v1.0.0-preview/.env.docker
...
# 5. 下载 Nginx 配置文件
mkdir -p nginx
curl -o nginx/nginx.conf https://raw.githubusercontent.com/hsliuping/柳暗花明/v1.0.0-preview/nginx/nginx.conf
...
```
NAME                       IMAGE                                    STATUS
lahm-backend      hsliup/lahm-backend:latest      Up (healthy)
lahm-frontend     hsliup/lahm-frontend:latest     Up (healthy)
lahm-mongodb      mongo:4.4                                Up (healthy)
lahm-nginx        nginx:alpine                             Up
lahm-redis        redis:7-alpine                           Up (healthy)
```
...
```powershell
# 查看数据卷
docker volume ls | Select-String lahm

# 备份数据卷
docker run --rm -v lahm_mongodb_data:/data -v ${PWD}:/backup alpine tar czf /backup/mongodb_backup.tar.gz /data

# 恢复数据卷
docker run --rm -v lahm_mongodb_data:/data -v ${PWD}:/backup alpine tar xzf /backup/mongodb_backup.tar.gz -C /
```
...
```powershell
# 检查后端日志
docker logs lahm-backend

# 检查 Nginx 日志
docker logs lahm-nginx
...
```bash
# 检查 MongoDB 状态
docker exec -it lahm-mongodb mongo -u admin -p lahm123 --authenticationDatabase admin

# 重启 MongoDB
docker-compose -f docker-compose.hub.nginx.yml restart mongodb

# 检查数据卷
docker volume inspect lahm_mongodb_data
```
...
services:
  redis:
    command: redis-server --appendonly yes --requirepass lahm123 --maxmemory 2gb --maxmemory-policy allkeys-lru
```
...
```bash
sudo nano /etc/logrotate.d/lahm
```

```
/path/to/lahm-demo/logs/*.log {
    daily
    rotate 7
    compress
    delaycompress
...
```bash
# 导出配置数据
docker exec -it lahm-backend python -c "
...
# 复制备份文件到宿主机
docker cp lahm-backend:/app/data/export_*.json ./backup/
```
...
- **GitHub Issues**: https://github.com/hsliuping/柳暗花明/issues
- **文档**: https://github.com/hsliuping/柳暗花明/tree/v1.0.0-preview/docs
- **示例**: https://github.com/hsliuping/柳暗花明/tree/v1.0.0-preview/examples
