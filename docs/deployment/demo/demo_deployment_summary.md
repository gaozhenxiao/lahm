...
本文档总结了为在远程服务器上部署 柳暗花明 演示系统而创建的完整解决方案。
...
# 或下载必要文件
mkdir -p 柳暗花明-Demo/{install,scripts}
cd 柳暗花明-Demo
...
| 服务 | 镜像 | 说明 |
|------|------|------|
| Frontend | `hsliup/lahm-frontend:latest` | Vue 3 前端 |
| Backend | `hsliup/lahm-backend:latest` | FastAPI 后端 |
...
| 数据卷 | 挂载点 | 说明 |
|--------|--------|------|
| `lahm_mongodb_data` | `/data/db` | MongoDB 数据 |
| `lahm_redis_data` | `/data` | Redis 数据 |
...
# 查看后端日志
docker logs lahm-backend | grep "配置桥接"
```
...
现在您可以使用这些文件和脚本在任何远程服务器上快速部署 柳暗花明 演示系统。
