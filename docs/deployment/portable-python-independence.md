...
2. **解压到 vendors 目录**
   ```powershell
   $pythonDir = "release\Lahm-portable\vendors\python"
...
# 打包后端
pyinstaller --onefile --name lahm-backend app/main.py

# 打包 worker
pyinstaller --onefile --name lahm-worker app/worker.py
```
...
1. **下载绿色版**
   ```
   Lahm-Portable-v1.0.0.zip (430 MB)
   ```
