...
- `MONGODB_HOST=127.0.0.1`、`MONGODB_PORT=27017`、`MONGODB_DATABASE=lahm`
...
在项目根目录运行：
```
powershell -ExecutionPolicy Bypass -File scripts/deployment/assemble_portable_release.ps1 -ReleaseDir .\release\Lahm-portable -BuildFrontend -RecreateVenv
```
执行后，`release/Lahm-portable` 目录即为可分发的便携版安装包。