...
说明
- 目标：对 `lahm/` 目录进行结构化体检，沉淀一份可执行的优化清单，后续作为改进参考。
- 范围：`lahm` 下的 agents/api/config/dataflows/graph/llm(_adapters)/tools/utils 等模块。
...
- App 缓存优先化开关（ta_use_app_cache）
  ...
  - 行为：开启后 柳暗花明 优先读 App 缓存（Mongo 集合 `stock_basic_info`/`market_quotes`），未命中回退直连
