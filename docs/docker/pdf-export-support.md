...
柳暗花明 的 Docker 镜像已经内置了完整的 PDF 导出支持，包括：
...
# 或手动构建
docker build -f Dockerfile.backend -t lahm-backend:latest .
```
...
```bash
docker exec lahm-test python -c "
import weasyprint
...
```bash
docker exec lahm-test pandoc --version
```

#### 5. 检查 wkhtmltopdf

```bash
docker exec lahm-test wkhtmltopdf --version
```

#### 6. 停止容器

```bash
docker stop lahm-test
```
...
4. **清理缓存重新构建**：
   ```bash
   docker build --no-cache -f Dockerfile.backend -t lahm-backend:latest .
   ```
...
```bash
# 启动容器
docker run --rm -it lahm-backend:latest bash
