...
```bash
# 使用 scp
scp database_export_config_2025-10-16.json user@new-server:/path/to/柳暗花明/
...
📝 后续步骤:
   1. 重启后端服务: docker restart lahm-backend
   2. 访问前端并使用默认账号登录
   3. 检查系统配置是否正确加载
```
...
用户名          邮箱                           角色       状态       创建时间
------------------------------------------------------------------------------------------
admin           admin@lahm.cn         管理员     激活       2025-10-16 10:30
...
```python
MONGO_URI = "mongodb://admin:lahm123@localhost:27017/lahm?authSource=admin"
DB_NAME = "lahm"
```
...
# 方法 3：手动删除用户
docker exec -it lahm-mongodb mongo lahm \
...
# 2. 检查后端日志
docker logs lahm-backend --tail 100
...
docker cp lahm-mongodb:/tmp/backup ./backup_before_import
```
