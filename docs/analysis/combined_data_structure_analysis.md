...
### 核心文件
- **工具定义**: `lahm/agents/utils/agent_utils.py` (第 770-1164 行)
  - `get_stock_fundamentals_unified()` 统一基本面分析工具

- **A股数据处理**: `lahm/dataflows/optimized_china_data.py`
  - `OptimizedChinaDataProvider` 类
  - `_get_real_financial_metrics()` - MongoDB优先的财务数据获取
  - `_generate_fundamentals_report()` - 基本面报告生成

### MongoDB 相关
- **MongoDB缓存适配器**: `lahm/dataflows/cache/mongodb_cache_adapter.py`
  - `get_stock_basic_info()` - 按数据源优先级获取基础信息
  - `get_financial_data()` - 按数据源优先级获取财务数据
  - `_get_data_source_priority()` - 获取数据源优先级配置

- **数据库管理**: `lahm/config/database_manager.py`
  - MongoDB 连接管理
  - 数据库可用性检查

### 其他市场
- **港股数据**: `lahm/dataflows/providers/hk/improved_hk.py`
- **美股数据**: `lahm/dataflows/interface.py`
- **数据源管理**: `lahm/dataflows/data_source_manager.py`

### 配置相关
- **运行时配置**: `lahm/config/runtime_settings.py`
