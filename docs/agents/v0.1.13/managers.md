...
管理层团队是 柳暗花明 框架的决策核心，负责协调各个智能体的工作流程，评估投资辩论，并做出最终的投资决策。管理层通过综合分析师、研究员、交易员和风险管理团队的输出，形成全面的投资策略和具体的执行计划。
...
```python
# 统一的管理层模块日志装饰器
from lahm.utils.tool_logging import log_manager_module
...
**文件位置**: `lahm/agents/managers/portfolio_manager.py`
...
1. **创建新管理角色**
```python
# lahm/agents/managers/new_manager.py
from lahm.utils.tool_logging import log_manager_module
from lahm.utils.logging_init import get_logger
...
```python
# 在图配置中添加新管理角色
from lahm.agents.managers.new_manager import create_new_manager
...
管理层团队作为柳暗花明框架的决策中枢，通过科学的决策流程和全面的信息整合，确保投资决策的质量和有效性，为投资组合的成功管理提供强有力的领导和指导。