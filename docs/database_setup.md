...
柳暗花明现在支持MongoDB和Redis数据库，提供数据持久化存储和高性能缓存功能。
...
2. **连接失败**
   ...
   docker logs lahm-redis
   ```

3. **权限问题**
   ```bash
   # 重启容器
   docker restart lahm-mongodb lahm-redis
   ```
...
```bash
# 停止并删除容器
docker stop lahm-mongodb lahm-redis lahm-redis-commander
docker rm lahm-mongodb lahm-redis lahm-redis-commander
