...
| 接口 | 文件 | 影响 | 优先级 |
|-----|------|------|--------|
| **分析数据流** | `lahm/dataflows/optimized_china_data.py` | 分析报告中的PE/PB | 🔴 高 |
...
**文件**：`lahm/dataflows/realtime_metrics.py`（新建）
...
async def enrich_results_with_realtime_metrics(self, results: List[Dict]) -> List[Dict]:
    """为筛选结果添加实时PE/PB"""
    from lahm.dataflows.realtime_metrics import calculate_realtime_pe_pb
...
**文件**：`lahm/dataflows/optimized_china_data.py`
...
```python
import pytest
from lahm.dataflows.realtime_metrics import (
