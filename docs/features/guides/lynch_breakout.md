# 林奇GARP突破（lynch_breakout）

高增长+合理PE分位，突破60日高。

标签：`林奇` · `GARP` · `突破`

## 怎么选股（逐步）

1. **股票池**：默认 `hs300` 成分；对每只股票逐日检查，下列条件需同时满足才开仓。
2. **估值闸门**：PE 历史分位 ≤ **55.00%**（窗口 `756` 交易日）
3. **成长闸门**：净利/营收同比 ≥ **20.00%**
4. **图形·突破确认**：收盘 ≥ 昨日起算 **60** 日最高价，且收盘 > **MA20**
5. **组合约束**：最多同时持有 **8** 只，等权；有空位才开新仓
6. **出场（任一触发）**：止损 **−12.00%**；持有满 **20** 个交易日

信号实现：`signal_lynch_breakout`（`app/services/factors/signal_specs.py`）。

## 举例：回测里真实成交的一笔

来源：`data/factors/lynch_breakout_trade_history.csv`

| 项目 | 内容 |
|---|---|
| 标的 | **sz.300308** |
| 开仓 | 2016-10-13，约 9.953 元 |
| 清仓 | 2016-11-09，约 14.48 元 |
| 单笔涨跌 | **45.45%** |
| 当日组合贡献 | NAV 7.50% |
| 出场备注 | hold_end；买入2016-10-13 成本价9.9529 |

**开仓信号备注**：林奇GARP突破(YOYNI)

解读：开仓日报价满足「财务闸门 + 技术图形」；清仓由止盈/止损/到期之一触发。案例用于理解规则，不构成推荐。

## 怎么跑

```bash
python scripts/run_new_factors.py --only lynch_breakout --limit 40
python scripts/run_new_factors.py --only lynch_breakout --limit 0
```

产物：`data/factors/lynch_breakout_*`
