# 双改善突破(lag28 np0.11)（dual_lag28_np11）
...
来源：`data/factors/dual_lag28_np11_trade_history.csv`
...
```bash
python scripts/run_new_factors.py --only dual_lag28_np11 --limit 40
python scripts/run_new_factors.py --only dual_lag28_np11 --limit 0
```

产物：`data/factors/dual_lag28_np11_*`
