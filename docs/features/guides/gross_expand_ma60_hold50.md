# 毛利率扩张站上MA60(持有50)（gross_expand_ma60_hold50）
...
```bash
python scripts/run_new_factors.py --only gross_expand_ma60_hold50 --limit 40
python scripts/run_new_factors.py --only gross_expand_ma60_hold50 --limit 0
```

产物：`data/factors/gross_expand_ma60_hold50_*`
