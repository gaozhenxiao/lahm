# 归母领先突破(滞后29)（parent_lead_lag29_brk60）
...
来源：`data/factors/parent_lead_lag29_brk60_trade_history.csv`
...
```bash
python scripts/run_new_factors.py --only parent_lead_lag29_brk60 --limit 40
python scripts/run_new_factors.py --only parent_lead_lag29_brk60 --limit 0
```

产物：`data/factors/parent_lead_lag29_brk60_*`
