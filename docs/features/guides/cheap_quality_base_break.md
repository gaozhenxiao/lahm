# 低估质量横盘突破（cheap_quality_base_break）

低PB + 高ROE 横盘后突破。

标签：`基本面` · `技术面` · `估值` · `质量`

## 怎么选股（逐步）

1. **股票池**：默认 `hs300` 成分；对每只股票逐日检查，下列条件需同时满足才开仓。
2. **估值闸门**：PB 历史分位 ≤ **35.00%**（窗口 `756` 交易日）
3. **质量底线**：ROE ≥ **10.00%**
4. **图形·收窄后突破**：振幅 ≤ 22.00% 的横盘背景下，收盘 ≥ 昨日起算 **60** 日高，且 > MA20
5. **组合约束**：最多同时持有 **8** 只，等权；有空位才开新仓
6. **出场（任一触发）**：止损 **−12.00%**；持有满 **30** 个交易日

信号实现：`signal_cheap_quality_base_break`（`app/services/factors/signal_specs.py`）。

## 举例：回测里真实成交的一笔

来源：`data/factors/cheap_quality_base_break_trade_history.csv`

| 项目 | 内容 |
|---|---|
| 标的 | **sh.600346** |
| 开仓 | 2019-02-25，约 8.862 元 |
| 清仓 | 2019-04-09，约 11.3 元 |
| 单笔涨跌 | **27.46%** |
| 当日组合贡献 | NAV 3.43% |
| 出场备注 | hold_end；买入2019-02-25 成本价8.8621 |

**开仓信号备注**：低估高质量横盘突破

解读：开仓日报价满足「财务闸门 + 技术图形」；清仓由止盈/止损/到期之一触发。案例用于理解规则，不构成推荐。

## 怎么跑

```bash
python scripts/run_new_factors.py --only cheap_quality_base_break --limit 40
python scripts/run_new_factors.py --only cheap_quality_base_break --limit 0
```

产物：`data/factors/cheap_quality_base_break_*`
