...
4. **图形·突破确认**：收盘 ≥ 昨日起算 **90** 日最高价，且收盘 > **MA20**
...
```bash
python scripts/run_new_factors.py --only catchup_brk90 --limit 40
python scripts/run_new_factors.py --only catchup_brk90 --limit 0
```

产物：`data/factors/catchup_brk90_*`
