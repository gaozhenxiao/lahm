# 轻资产金叉（asset_light_trend）

净利高增且资产增速低，MA20上穿MA60。

标签：`基本面` · `技术面` · `轻资产`

## 怎么选股（逐步）

1. **股票池**：默认 `hs300` 成分；对每只股票逐日检查，下列条件需同时满足才开仓。
2. **成长闸门**：净利/营收同比 ≥ **12.00%**
3. **图形·突破确认**：收盘 ≥ 昨日起算 **60** 日最高价，且收盘 > **MA20**
4. **组合约束**：最多同时持有 **8** 只，等权；有空位才开新仓
5. **出场（任一触发）**：止损 **−12.00%**；持有满 **30** 个交易日

信号实现：`signal_asset_light_trend`（`app/services/factors/signal_specs.py`）。

## 举例：回测里真实成交的一笔

来源：`data/factors/asset_light_trend_trade_history.csv`

| 项目 | 内容 |
|---|---|
| 标的 | **sh.600221** |
| 开仓 | 2024-10-09，约 1.19 元 |
| 清仓 | 2024-11-20，约 1.81 元 |
| 单笔涨跌 | **52.10%** |
| 当日组合贡献 | NAV 6.51% |
| 出场备注 | hold_end；买入2024-10-09 成本价1.1900 |

**开仓信号备注**：轻资产高增均线金叉

解读：开仓日报价满足「财务闸门 + 技术图形」；清仓由止盈/止损/到期之一触发。案例用于理解规则，不构成推荐。

## 怎么跑

```bash
python scripts/run_new_factors.py --only asset_light_trend --limit 40
python scripts/run_new_factors.py --only asset_light_trend --limit 0
```

产物：`data/factors/asset_light_trend_*`
