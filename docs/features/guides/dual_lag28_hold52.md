# 双改善突破(lag28持有52)（dual_lag28_hold52）
...
来源：`data/factors/dual_lag28_hold52_trade_history.csv`
...
```bash
python scripts/run_new_factors.py --only dual_lag28_hold52 --limit 40
python scripts/run_new_factors.py --only dual_lag28_hold52 --limit 0
```

产物：`data/factors/dual_lag28_hold52_*`
