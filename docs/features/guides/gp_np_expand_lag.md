...
来源：`data/factors/gp_np_expand_lag_trade_history.csv`
...
```bash
python scripts/run_new_factors.py --only gp_np_expand_lag --limit 40
python scripts/run_new_factors.py --only gp_np_expand_lag --limit 0
```

产物：`data/factors/gp_np_expand_lag_*`
