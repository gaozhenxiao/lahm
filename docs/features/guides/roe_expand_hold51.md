# ROE扩张突破(持有51)（roe_expand_hold51）
...
```bash
python scripts/run_new_factors.py --only roe_expand_hold51 --limit 40
python scripts/run_new_factors.py --only roe_expand_hold51 --limit 0
```

产物：`data/factors/roe_expand_hold51_*`
