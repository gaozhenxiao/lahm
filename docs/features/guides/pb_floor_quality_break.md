# PB低位质量突破（pb_floor_quality_break）

PB历史低分位 + ROE底线后突破并站上均线。
...
```bash
python scripts/run_new_factors.py --only pb_floor_quality_break --limit 40
python scripts/run_new_factors.py --only pb_floor_quality_break --limit 0
```

产物：`data/factors/pb_floor_quality_break_*`
