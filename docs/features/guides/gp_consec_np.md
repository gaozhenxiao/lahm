# 毛利率连续扩张(净利)（gp_consec_np）

连续毛利扩张 + 净利率底线后突破。

标签：`基本面` · `技术面` · `毛利率` · `净利率` · `新结构`
...
```bash
python scripts/run_new_factors.py --only gp_consec_np --limit 40
python scripts/run_new_factors.py --only gp_consec_np --limit 0
```

产物：`data/factors/gp_consec_np_*`
