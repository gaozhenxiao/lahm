# 双改善横盘突破(更严)（dual_improve_base_tight2）
...
4. **图形·收窄后突破**：振幅 ≤ 15.00% 的横盘背景下，收盘 ≥ 昨日起算 **60** 日高，且 > MA20
5. **组合约束**：最多同时持有 **8** 只，等权；有空位才开新仓
6. **出场（任一触发）**：止损 **−10.00%**；持有满 **25** 个交易日
...
```bash
python scripts/run_new_factors.py --only dual_improve_base_tight2 --limit 40
python scripts/run_new_factors.py --only dual_improve_base_tight2 --limit 0
```

产物：`data/factors/dual_improve_base_tight2_*`
