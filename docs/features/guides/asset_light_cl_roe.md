# 轻资产合同负债(ROE)（asset_light_cl_roe）

轻资产预收突破 + ROE≥8%。

标签：`基本面` · `技术面` · `合同负债` · `ROE` · `新结构`
...
```bash
python scripts/run_new_factors.py --only asset_light_cl_roe --limit 40
python scripts/run_new_factors.py --only asset_light_cl_roe --limit 0
```

产物：`data/factors/asset_light_cl_roe_*`
