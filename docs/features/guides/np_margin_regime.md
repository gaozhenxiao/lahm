# 净利率体制切换（np_margin_regime）

净利率上穿近几期中枢，且估值不过贵。

标签：`基本面` · `净利率` · `体制`

## 怎么选股（逐步）

1. **股票池**：默认 `hs300` 成分；对每只股票逐日检查，下列条件需同时满足才开仓。
2. **估值闸门**：PE 历史分位 ≤ **70.00%**（窗口 `756` 交易日）
3. **利润率水平**：毛利率（或规则指定利润率）≥ **8.00%**
4. **图形·突破确认**：收盘 ≥ 昨日起算 **60** 日最高价，且收盘 > **MA20**
5. **组合约束**：最多同时持有 **8** 只，等权；有空位才开新仓
6. **出场（任一触发）**：止损 **−12.00%**；持有满 **25** 个交易日

信号实现：`signal_np_margin_regime`（`app/services/factors/signal_specs.py`）。

## 举例：回测里真实成交的一笔

来源：`data/factors/np_margin_regime_trade_history.csv`

| 项目 | 内容 |
|---|---|
| 标的 | **sh.600489** |
| 开仓 | 2025-08-29，约 15.97 元 |
| 清仓 | 2025-10-13，约 25.5 元 |
| 单笔涨跌 | **59.67%** |
| 当日组合贡献 | NAV 7.46% |
| 出场备注 | hold_end；买入2025-08-29 成本价15.9700 |

**开仓信号备注**：净利率上穿历史中枢

解读：开仓日报价满足「财务闸门 + 技术图形」；清仓由止盈/止损/到期之一触发。案例用于理解规则，不构成推荐。

## 怎么跑

```bash
python scripts/run_new_factors.py --only np_margin_regime --limit 40
python scripts/run_new_factors.py --only np_margin_regime --limit 0
```

产物：`data/factors/np_margin_regime_*`
