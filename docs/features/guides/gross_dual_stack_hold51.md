# 毛利率ROE双改善(持有51)（gross_dual_stack_hold51）
...
```bash
python scripts/run_new_factors.py --only gross_dual_stack_hold51 --limit 40
python scripts/run_new_factors.py --only gross_dual_stack_hold51 --limit 0
```

产物：`data/factors/gross_dual_stack_hold51_*`
