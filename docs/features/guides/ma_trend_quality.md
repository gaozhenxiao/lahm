# 质量趋势金叉（ma_trend_quality）

ROE不差且估值不过贵时，MA20上穿MA60形成多头。
...
2. **估值闸门**：PE 历史分位 ≤ **80.00%**（窗口 `756` 交易日）
3. **质量底线**：ROE ≥ **8.00%**
4. **图形·突破确认**：收盘 ≥ 昨日起算 **60** 日最高价，且收盘 > **MA20**
5. **组合约束**：最多同时持有 **8** 只，等权；有空位才开新仓
6. **出场（任一触发）**：止损 **−12.00%**；持有满 **25** 个交易日

信号实现：`signal_ma_trend_quality`（`app/services/factors/signal_specs.py`）。

## 举例：回测里真实成交的一笔

来源：`data/factors/ma_trend_quality_trade_history.csv`
