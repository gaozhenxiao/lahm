# 双改善突破(lag28 np0.10持有51)（dual_lag28_np10）
...
来源：`data/factors/dual_lag28_np10_trade_history.csv`
...
```bash
python scripts/run_new_factors.py --only dual_lag28_np10 --limit 40
python scripts/run_new_factors.py --only dual_lag28_np10 --limit 0
```

产物：`data/factors/dual_lag28_np10_*`
