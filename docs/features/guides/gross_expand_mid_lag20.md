# 毛利率中档扩张(热窗20)（gross_expand_mid_lag20）

毛利率扩张后突破：毛利≥20%；毛利环比升≥0.60%；财务热窗20日；入场=60日高+MA20；持有50日；止损12%；无固定止盈。
...
```bash
python scripts/run_new_factors.py --only gross_expand_mid_lag20 --limit 40
python scripts/run_new_factors.py --only gross_expand_mid_lag20 --limit 0
```

产物：`data/factors/gross_expand_mid_lag20_*`
