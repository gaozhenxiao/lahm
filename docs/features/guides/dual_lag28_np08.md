# 双改善突破(lag28 np0.08)（dual_lag28_np08）
...
来源：`data/factors/dual_lag28_np08_trade_history.csv`
...
```bash
python scripts/run_new_factors.py --only dual_lag28_np08 --limit 40
python scripts/run_new_factors.py --only dual_lag28_np08 --limit 0
```

产物：`data/factors/dual_lag28_np08_*`
