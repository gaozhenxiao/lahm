# 高净利率突破(持有50)（high_margin_break_hold50）
...
```bash
python scripts/run_new_factors.py --only high_margin_break_hold50 --limit 40
python scripts/run_new_factors.py --only high_margin_break_hold50 --limit 0
```

产物：`data/factors/high_margin_break_hold50_*`
