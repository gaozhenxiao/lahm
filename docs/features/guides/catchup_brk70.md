...
4. **图形·突破确认**：收盘 ≥ 昨日起算 **70** 日最高价，且收盘 > **MA20**
...
来源：`data/factors/catchup_brk70_trade_history.csv`
...
```bash
python scripts/run_new_factors.py --only catchup_brk70 --limit 40
python scripts/run_new_factors.py --only catchup_brk70 --limit 0
```

产物：`data/factors/catchup_brk70_*`
