# 优质低波突破(持有50)（quality_coil_hold50）
...
5. **出场（任一触发）**：止损 **−12.00%**；持有满 **50** 个交易日
...
```bash
python scripts/run_new_factors.py --only quality_coil_hold50 --limit 40
python scripts/run_new_factors.py --only quality_coil_hold50 --limit 0
```

产物：`data/factors/quality_coil_hold50_*`
