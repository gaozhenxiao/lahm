...
本指南提供了完整的 LLM 适配器测试流程，确保新集成的大模型能够稳定运行并正确集成到 柳暗花明 系统中。
...
### 2. 工具调用测试
验证适配器能够正确执行 function calling，这是 柳暗花明 分析功能的核心。
...
from lahm.llm_adapters.openai_compatible_base import OPENAI_COMPATIBLE_PROVIDERS
...
# 检查 __init__.py 导出
python -c "from lahm.llm_adapters import ChatYourProvider; print('导入成功')"
```
...
通过遵循这个完整的测试指南，您可以确保新集成的 LLM 适配器质量可靠，功能完整，能够稳定地为 柳暗花明 用户提供服务。