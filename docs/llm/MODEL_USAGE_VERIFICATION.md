...
在分析过程中，你还会看到 柳暗花明 库的日志，显示实际调用的模型：
...
4. ✅ **引擎成功使用**：LahmGraph 实例使用了正确的模型配置
