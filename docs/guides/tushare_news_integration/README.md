...
柳暗花明系统已成功集成Tushare新闻接口，提供了业界领先的多源新闻数据获取能力。
...
```python
from lahm.dataflows.providers.tushare_provider import get_tushare_provider
...
```python
import logging
logging.getLogger('lahm.dataflows.providers.base_provider.Tushare').setLevel(logging.DEBUG)
```
