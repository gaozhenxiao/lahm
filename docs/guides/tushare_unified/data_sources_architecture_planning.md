...
**1. 迁移app/services/data_sources到lahm**:
```bash
# 迁移步骤
mkdir -p lahm/dataflows/providers
mv app/services/data_sources/* lahm/dataflows/providers/
```
...
```python
# 全局替换导入路径
from app.services.data_sources.xxx → from lahm.dataflows.providers.xxx
```
...
```
外部数据源 → lahm适配器 → app同步服务 → MongoDB → app业务服务 → API/前端
```
