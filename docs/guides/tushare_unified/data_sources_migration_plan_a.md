...
```python
# lahm/dataflows/providers/base_provider.py
...
from lahm.dataflows.manager import DataSourceManager
...
**任务清单**:
- [ ] 创建 `lahm/dataflows/providers/` 目录
...
**每个适配器的迁移步骤**:
- [ ] 分析现有实现 (`app/services/data_sources/` 和 `lahm/dataflows/`)
- [ ] 创建新的统一适配器 (`lahm/dataflows/providers/xxx_provider.py`)
- [ ] 实现统一接口方法
- [ ] 迁移数据标准化逻辑
- [ ] 编写单元测试
- [ ] 集成测试验证
...
```bash
# 创建目录结构
mkdir -p lahm/dataflows/providers
touch lahm/dataflows/providers/__init__.py

# 创建基础文件
touch lahm/dataflows/providers/base_provider.py
touch lahm/dataflows/manager.py
touch lahm/dataflows/config.py
```
...
- [ ] 分析现有实现差异
  ...
  - `lahm/dataflows/tushare_adapter.py`
...
# 创建__init__.py文件
cat > lahm/dataflows/providers/__init__.py << 'EOF'
"""
统一数据源提供器包
"""
...
files=(
    "lahm/dataflows/providers/base_provider.py"
    "lahm/dataflows/providers/tushare_provider.py"
    "lahm/dataflows/providers/akshare_provider.py"
    "lahm/dataflows/manager.py"
    "lahm/dataflows/config.py"
)
