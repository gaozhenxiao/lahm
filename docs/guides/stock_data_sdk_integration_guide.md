...
本指南详细说明如何在柳暗花明系统中接入新的股票数据SDK，包括架构设计、接入流程、代码规范和测试验证。
...
```
外部SDK → lahm适配器 → app数据同步服务 → MongoDB存储 → app查询服务 → API/前端
```
...
在 `lahm/dataflows/` 目录下创建新的SDK适配器：

```python
# lahm/dataflows/new_sdk_provider.py
...
from lahm.dataflows.new_sdk_provider import NewSDKProvider

class NewSDKSyncService:
    """新SDK数据同步服务 - 负责数据库操作和业务逻辑"""

    def __init__(self):
        self.provider = NewSDKProvider()  # 使用lahm的适配器
...
```bash
# 复制示例适配器 (lahm层 - 纯数据获取)
cp lahm/dataflows/example_sdk_provider.py lahm/dataflows/your_sdk_provider.py
...
```python
# 修改 lahm/dataflows/your_sdk_provider.py
...
python -c "
import asyncio
from lahm.dataflows.your_sdk_provider import YourSDKProvider
...
- [基础提供器接口](../../lahm/dataflows/base_provider.py)
- [示例SDK适配器](../../lahm/dataflows/example_sdk_provider.py)
