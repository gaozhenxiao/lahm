...
柳暗花明 v0.1.7 引入了完整的Docker容器化部署方案，让您可以通过一条命令启动完整的股票分析环境。本指南将详细介绍如何使用Docker部署和管理柳暗花明。
...
**重要说明**: 柳暗花明目前不提供预构建的Docker镜像，需要在本地构建。
...
# - 柳暗花明-mongodb (数据库)
# - 柳暗花明-redis (缓存)
# - 柳暗花明-mongo-express (数据库管理)
# - 柳暗花明-redis-commander (缓存管理)
```
...
```bash
# 备份数据
docker exec 柳暗花明-mongodb mongodump --out /backup
docker exec 柳暗花明-redis redis-cli BGSAVE

# 清理缓存
docker exec 柳暗花明-redis redis-cli FLUSHALL

# 查看数据使用情况
docker exec 柳暗花明-mongodb mongo --eval "db.stats()"
```
...
```bash
# 检查数据库容器状态
docker logs 柳暗花明-mongodb

# 检查网络连接
docker exec 柳暗花明-web ping mongodb
...
```bash
# 检查环境变量
docker exec 柳暗花明-web env | grep API_KEY
...
# 每周执行一次
# 1. 备份数据
docker exec 柳暗花明-mongodb mongodump --out /backup/$(date +%Y%m%d)
