...
本文档定义运行时配置的“单一事实来源（SoT）”、优先级、边界与迁移路线，适用于 app 与 lahm 两侧。
...
柳暗花明 侧部分限速/睡眠参数支持通过后端系统设置统一管理，亦可通过环境变量覆盖；优先级：DB(system_settings) > ENV > 代码默认。
