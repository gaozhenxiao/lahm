...
用户反馈：警告日志和错误日志混在 `lahm.log` 中，不方便人工查找和排查问题。

**需求**：
- 将 WARNING、ERROR、CRITICAL 级别的日志单独输出到 `error.log`
- 保持原有的 `lahm.log` 记录所有级别的日志
- 方便快速定位和监控问题
...
**文件**：`lahm/utils/logging_manager.py`
...
```bash
# 需要在所有日志中搜索错误
grep "ERROR\|WARNING" logs/lahm.log
```
