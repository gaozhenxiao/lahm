<template>
  <div class="factors-page">
    ...
    <el-card shadow="never">
      ...
      >
        ...
        <el-table-column label="操作" width="100" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" @click.stop="openGuide(row)">说明</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    <el-drawer
      ...
      :title="preview.title"
      :size="preview.kind === 'detail' ? '78%' : '640px'"
      destroy-on-close
      class="factor-preview-drawer"
      @closed="revokePreview"
    >
      <div v-loading="preview.loading" class="preview-body">
        <template v-if="preview.kind === 'detail'">
          <el-tabs v-model="preview.detailTab" class="detail-tabs">
            ...
            <el-tab-pane label="操作流水" name="flow">
              ...
              <el-table :data="preview.trades" stripe height="520" size="small" empty-text="暂无交易流水">
                <el-table-column
            ...
            <el-tab-pane label="持仓分布" name="holdings">
              <div class="trades-meta contrib-toolbar">
                <span>
                  ...
                  当日持仓 {{ preview.endHoldings.length }} 只（含当日买入） ·
                  合计仓位 {{ fmtPct(preview.endHoldingsTotalWeight) }}
...
const updateSettings = ref<{
  kline?: { enabled: boolean; cron: string; times_hint: string }
  financial?: { enabled: boolean; cron: string; times_hint: string }
...
const preview = reactive({
  ...
  tradeTotal: 0,
  detailTab: 'equity' as 'equity' | 'flow' | 'contrib' | 'holdings',
  ...
  /** 回测末日持仓（持仓分布 Tab） */
  endHoldings: [] as OpenHolding[],
  endHoldingsTotalWeight: 0,
...
function onEquityPreviewKeydown(e: KeyboardEvent) {
  if (
    !preview.visible ||
    preview.kind !== 'detail' ||
    preview.detailTab !== 'equity' ||
    !preview.equitySeries.length
  )
    return
...
async function applyFactorDeepLink() {
  ...
  try {
    ...
    if (open === 'trades') {
      ...
      await openFactorDetail(row, 'flow')
    }
  } finally {
    deepLinkBusy = false
  }
...
function revokePreview() {
  ...
  preview.endHoldings = []
  preview.endHoldingsTotalWeight = 0
...
function onFactorRowClick(row: FactorItem) {
  const arts = availableArtifacts(row)
  const equity =
    arts.find((a) => a.kind === 'image' && !String(a.id).includes('share')) ||
    arts.find((a) => a.kind === 'image')
  const trade = pickTradeArtifact(row)
  if (equity || trade) {
    openFactorDetail(row, 'equity')
    return
  }
  if (arts[0]) {
    openArtifact(row, arts[0])
    return
  }
  openGuide(row)
}

/** 统一打开因子详情抽屉：净值 + 交易相关 Tab */
async function openFactorDetail(
  row: FactorItem,
  initialTab: 'equity' | 'flow' | 'contrib' | 'holdings' = 'equity',
) {
  revokePreview()
  preview.visible = true
  preview.loading = true
  preview.title = `${row.name} · 回测详情`
  preview.kind = 'detail'
  preview.detailTab = initialTab
  preview.factorId = row.factor_id

  const arts = availableArtifacts(row)
  const equityArt =
    arts.find((a) => a.kind === 'image' && isEquityCurveArtifact(a)) ||
    arts.find((a) => a.kind === 'image' && !String(a.id).includes('share'))
  const tradeArt = pickTradeArtifact(row)
  preview.artifactId = (initialTab === 'flow' ? tradeArt?.id : equityArt?.id) || tradeArt?.id || equityArt?.id || ''
  preview.filename = preview.artifactId

  try {
    // 净值图静态 PNG（无日度序列时回退）
    if (equityArt) {
      try {
        const blob = await factorsApi.artifactBlob(row.factor_id, equityArt.id)
    ...
    // 日度净值序列
    const dailyId =
      (equityArt && resolveDailyArtifactIdForEquity(row, equityArt)) ||
      (tradeArt && resolveDailyArtifactId(row, tradeArt)) ||
      null
    if (dailyId) {
      try {
    ...
    if (tradeId) {
      try {
        ...
        const asOfHint = preview.posSeries[0]?.date || ''
        const endOpen = buildOpenHoldings(enriched, asOfHint, {
          totalExposure: preview.posSeries[0]?.position,
          nPos: preview.posSeries[0]?.n_pos,
        })
        preview.endHoldings = endOpen.holdings
        preview.endHoldingsTotalWeight = endOpen.totalWeight
        preview.holdingsAsOf = endOpen.asOf
      } catch {
        preview.equityTradeRows = []
        preview.trades = []
      }
    ...
    } else if (!preview.url && !preview.trades.length) {
      ElMessage.warning('暂无净值或交易数据')
    }
  } catch (e: any) {
...
async function openArtifact(row: FactorItem, a: FactorArtifact) {
  // 净值图 / 交易产物 → 统一详情抽屉（多 Tab）
  if ((a.kind === 'image' && isEquityCurveArtifact(a)) || isTradeArtifact(a)) {
    await openFactorDetail(row, isTradeArtifact(a) ? 'flow' : 'equity')
    return
  }
...
async function downloadCurrent() {
  ...
  try {
    if (preview.kind === 'detail' && preview.detailTab === 'contrib') {
      downloadContributionCsv()
      ElMessage.success('已下载净值贡献表')
      return
    }
    if (preview.kind === 'detail' && preview.detailTab === 'holdings') {
      downloadHoldingsCsv()
...
.factor-preview-drawer :deep(.el-drawer__body) {
  padding-top: 8px;
  display: flex;
  flex-direction: column;
}
