...
const saveAsLeads = async () => {
  ...
  try {
    const data = await leadsApi.fromScreening({
      items: selectedStocks.value.map((s: any) => ({
        code: s.code || s.symbol,
        name: s.name,
        market: s.market || 'CN',
        industry: s.industry,
        close: s.close,
        pct_chg: s.pct_chg || s.changePercent,
      })),
      reason: '来自股票筛选',
      tags: ['screening'],
      score: 60,
    })
    ElMessage.success(`已写入机会列表 ${data?.created ?? selectedStocks.value.length} 条`)
    router.push('/leads')
  } catch (e: any) {
    ElMessage.error(e?.message || '保存机会失败')
  } finally {
