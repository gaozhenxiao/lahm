...
def test_news_analyst_integration():
    ...
    try:
        # 1. 检查新闻分析师的工具绑定
        print(f"\n📰 第一步：检查新闻分析师的工具绑定...")
        from lahm.agents.analysts.news_analyst import create_news_analyst
        ...
        # 读取新闻分析师源码
        news_analyst_file = "lahm/agents/analysts/news_analyst.py"
        try:
            ...
            # 检查关键集成点
            integration_checks = [
                ("统一新闻工具导入", "from lahm.tools.unified_news_tool import create_unified_news_tool"),
        ...
        # 检查工作流程设置文件
        setup_file = "lahm/graph/setup.py"
        try:
            with open(setup_file, "r", encoding="utf-8") as f:
                setup_code = f.read()
            
            workflow_checks = [
                ("新闻分析师导入", "from lahm.agents.analysts.news_analyst import create_news_analyst"),
