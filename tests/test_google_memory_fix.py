...
def test_google_lahm_with_memory():
    """测试带内存的Google AI 柳暗花明"""
    try:
        print("\n🧪 测试带内存的Google AI 柳暗花明")
        print("=" * 60)
        ...
        # 创建LahmGraph实例
        print("🚀 初始化带内存的柳暗花明图...")
        graph = LahmGraph(["market"], config=config, debug=False)
...
def main():
    ...
    results['完整柳暗花明'] = test_google_lahm_with_memory()
    ...
    if successful_tests == total_tests:
        ...
        print("⚠️ 部分功能可用")
        if results['内存功能'] and not results['完整柳暗花明']:
