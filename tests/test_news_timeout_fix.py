...
from lahm.dataflows.googlenews_utils import getNewsData, make_request
...
class TestNewsTimeoutFix(unittest.TestCase):
    ...
    def test_news_source_fallback(self):
        """测试新闻源轮询机制"""
        # 模拟实时新闻聚合器失败
        with patch('lahm.dataflows.realtime_news_utils.RealtimeNewsAggregator.get_realtime_stock_news') as mock_aggregator:
            mock_aggregator.side_effect = Exception("模拟实时新闻聚合器失败")
            
            # 模拟Google新闻获取失败
            with patch('lahm.dataflows.interface.get_google_news') as mock_google_news:
                mock_google_news.side_effect = Exception("模拟Google新闻获取失败")
                
                # 模拟东方财富新闻获取成功
                with patch('lahm.dataflows.akshare_utils.get_stock_news_em') as mock_em_news:
