...
from lahm.utils.logging_manager import get_logger
...
def setup_cli_logging():
    ...
    """
    import logging
    from lahm.utils.logging_manager import get_logger_manager
    ...
    # 同时移除lahm日志器的控制台处理器
    lahm_logger = logging.getLogger('lahm')
    for handler in lahm_logger.handlers[:]:
        if isinstance(handler, logging.StreamHandler) and hasattr(handler, 'stream'):
            if handler.stream.name in ['<stderr>', '<stdout>']:
                lahm_logger.removeHandler(handler)
...
app = typer.Typer(
    name="柳暗花明",
    help="柳暗花明 CLI: 多智能体大语言模型金融交易框架 | Multi-Agents LLM Financial Trading Framework",
...
def update_display(layout, spinner_text=None):
    ...
    layout["header"].update(
        Panel(
            "[bold green]Welcome to 柳暗花明 CLI[/bold green]\n"
            "[dim]© [Tauric Research](https://github.com/TauricResearch)[/dim]",
            title="Welcome to 柳暗花明",
            border_style="green",
            padding=(1, 2),
            expand=True,
        )
...
def get_user_selections():
    ...
    welcome_content += "[bold green]柳暗花明: 多智能体大语言模型金融交易框架 - CLI[/bold green]\n"
    ...
    welcome_box = Panel(
        ...
        border_style="green",
        padding=(1, 2),
        title="欢迎使用 柳暗花明 | Welcome to 柳暗花明",
...
def config():
    ...
    """
    logger.info(f"\n[bold blue]🔧 柳暗花明 配置 | Configuration[/bold blue]")
...
def version():
    ...
    logger.info(f"\n[bold blue]📊 柳暗花明 版本信息 | Version Information[/bold blue]")
    ...
    logger.info(f"[green]创建目的 | Purpose:[/green] [cyan]更好地在中国推广柳暗花明[/cyan]")
    ...
    logger.info(f"  • 🎯 本项目旨在更好地在中国推广柳暗花明")
...
def data_config(
    ...
    """
    from lahm.config.config_manager import config_manager
    ...
    if reset:
        # 重置为默认配置
        default_data_dir = os.path.join(os.path.expanduser("~"), "Documents", "柳暗花明", "data")
    ...
    env_vars = {
        "LAHM_DATA_DIR": os.getenv("LAHM_DATA_DIR", "未设置"),
        "LAHM_CACHE_DIR": os.getenv("LAHM_CACHE_DIR", "未设置"),
        "LAHM_RESULTS_DIR": os.getenv("LAHM_RESULTS_DIR", "未设置")
    }
    ...
    logger.info(f"• 设置数据目录: lahm data-config --set /path/to/data")
    logger.info(f"• 重置为默认: lahm data-config --reset")
    logger.info(f"• 查看当前配置: lahm data-config --show")
...
def examples():
    ...
    """
    logger.info(f"\n[bold blue]📚 柳暗花明 示例程序 | Example Programs[/bold blue]")
...
def test():
    ...
    Run system tests
    """
    logger.info(f"\n[bold blue]🧪 柳暗花明 测试 | Tests[/bold blue]")
...
def help_chinese():
    ...
    """
    logger.info(f"\n[bold blue]📖 柳暗花明 中文帮助 | Chinese Help[/bold blue]")
